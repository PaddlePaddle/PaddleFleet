...
def _resolve_run_dir(path: str) -> str:
    ...
    if not candidates:
        sys.exit(f'目录下没有可解析的日志文件，也没有含日志文件的运行子目录: {path}')
...
def parse_dir(path: str) -> Dict[int, Dict[str, List[dict]]]:
    ...
    """
    out: Dict[int, Dict[str, List[dict]]] = defaultdict(lambda: {'per_token': [], 'final': []})
            ...
            for line in fh:
                ...
                for m in _PER_TOKEN_RE.finditer(line):
                    out[int(m.group('rank'))]['per_token'].append(
                        {'shape': m.group('shape').strip(), 'md5': m.group('md5')}
                    )
...
def compare(pf: Dict, mg: Dict, mb: int) -> int:
    ...
    if not ranks:
        sys.exit('两侧日志都没解析到 per_token_loss / final_loss，检查是否开启了 '
    ...
    n_mb = max(
        ...
        for r in ranks for k in ('per_token', 'final')
    )
    n_step = (n_mb + mb - 1) // mb

    print(f'ranks={ranks}  steps={n_step}  每 step microbatch={mb}')
    for r in ranks:
        print(f'  rank {r}: PF 记录 per_token={len(pf.get(r, empty)["per_token"])} '
        ...
        for rank in ranks:
            ...
            for mbi in range(mb):
                ...
                for kind, key, label in (
                    ...
                    extra = _diff_str(p, m, key) if (p and m and kind == 'final'
                                                     and not ok) else ''
                    pf_key = f'{key}={p[key]}' if p else '-'
                    mg_key = f'{key}={m[key]}' if m else '-'
                    print(f'      PF: md5={p["md5"] if p else "-":<34s} {pf_key}')
                    print(f'      MG: md5={m["md5"] if m else "-":<34s} {mg_key}{extra}')
        print()

    print('=' * 84)
    if bad_steps:
        print(f'  结论: 存在不一致，首个分叉 step = {min(bad_steps)}；'
              f'不一致 step 列表 = {sorted(bad_steps)}')
    else:
        print('  结论: 所有 rank / 所有 step 的 per_token_loss + final_loss md5 完全一致 ✅')
    print('=' * 84)
...
def main() -> int:
    ...
    ap.add_argument('pf_path', help='PF(Paddle) 日志目录或文件, 如 logs/paddle/20260731-172215；'
                                     '也可传 logs/paddle 自动取最新运行子目录')
    ap.add_argument('mg_path', help='MG(Torch) 日志目录或文件, 如 logs/torch/20260731-172403；'
                                     '也可传 logs/torch 自动取最新运行子目录')
    ap.add_argument('-m', '--microbatches', type=int, default=1,
