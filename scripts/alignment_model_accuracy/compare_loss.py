...
def main() -> int:
    ...
    ap.add_argument(
        "--loss-json",
        action="store_true",
        help="Compare complete native raw-loss JSON artifacts by IEEE bits.",
    )
    ap.add_argument(
        "--required-steps",
        type=int,
        help="Exact number of contiguous steps required in JSON mode.",
    )
    args = ap.parse_args()
    if args.loss_json:
        if args.required_steps is None or args.required_steps < 1:
            ap.error("--loss-json requires --required-steps >= 1")
        return compare_loss_json(
            args.pf_path, args.mg_path, args.required_steps
        )
    if args.required_steps is not None:
        ap.error("--required-steps requires --loss-json")
