...
class TrainTester:
    ...
    def run_training(
        ...
        cmd_str = f"paddlefleet-cli train {config_path} > {log_file} 2>&1"
    ...
    def run_export(
        ...
        cmd = ["paddlefleet-cli", "export", updated_config_path]
        if lora:
...
class TestTrain:
    ...
    def setup_class(self):
        ...
        subprocess.run(
            "pkill -9 -f 'paddlefleet/cli/launcher.py' 2>/dev/null || true",
            shell=True,
        )

        dist_log_dir = "./paddlefleet_dist_log"
