...
export log_path=/workspace/PaddleFleet/model_unittest_logs
export model_unittest_path=/workspace/PaddleFleet/scripts/regression
cd $nlp_dir
mkdir -p $log_path
...
for file_name in `git diff --numstat ${BRANCH} -- |awk '{print $NF}'`;do
    ...
    if [[ "$file_name" == "paddlefleet/transformers/"* ]] || [[ "$file_name" == "tests/transformers/"* ]]; then
        model_name=$(echo "$file_name" | sed -n 's#.*paddlefleet/transformers/\([^/]*\)/.*#\1#p')
