...
set -e
export formers_dir=/workspace/PaddleFleet
mkdir -p /workspace/PaddleFleet/build_logs
export log_path=/workspace/PaddleFleet/build_logs
mkdir -p /workspace/PaddleFleet/upload
upload_path=/workspace/PaddleFleet/upload
...
paddlefleet_build (){
    echo -e "\033[35m ---- build latest paddlefleet  \033[0m"
    cd $formers_dir
    rm -rf build/
    rm -rf dist/
    rm -rf paddlefleet.egg-info/
    ...
    python -m pip install --ignore-installed  PaddleFleet/dist/p****.whl --no-cache-dir --force-reinstall --no-dependencies
    cd -
    echo "waiting for import paddlefleet..."
    python -c "import paddlefleet; print('paddlefleet commit:',paddlefleet.version.commit)" >> ${log_path}/commit_info.txt
    commit=$(python -c "import paddlefleet; print(paddlefleet.version.commit)")
...
# main
cd ${formers_dir}
echo -e "\033[32m ---- build paddlefleet whl  \033[0m"
paddlefleet_build

echo -e "\033[32m ---- make PaddleFleet.tar.gz  \033[0m"
cd ${formers_dir}
...
if [ -n "$BRANCH" ] && [ "$BRANCH" = "develop" ]; then
    ...
    mv PaddleFleet.tar.gz ${upload_path}/
else
