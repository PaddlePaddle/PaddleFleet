...
if [[ "$update_baseline" == "true" ]]; then
    python PaddleFleet/tests/integration_test/check_loss.py \
...
python  PaddleFleet/tests/integration_test/check_loss.py --log_file ${case_name}.txt --gt_file ${case_gt_file}
exit_code=$?
if [ $exit_code -ne 0 ]; then
