...
def _initialize_affine_weight_cpu(
    ...
    *,
    ...
    master_weight = paddle.empty([output_size, input_size], dtype=paddle.float, requires_grad=False)
    init_method(master_weight)
    master_weight = master_weight.to(dtype=params_dtype)
    # Split and copy
    
    assert per_partition_size % stride == 0, f"{per_partition_size} is not divisible by {stride}"
    weight_list = paddle.split(master_weight, num_or_sections=stride, dim=partition_dim)
    if rank is None:
    ...
    with paddle.no_grad():
        ...
        cpu_weight = paddle.cat(my_weight_list, dim=partition_dim).to_dense()
...
class VocabParallelEmbedding(paddle.nn.Layer):
    ...
    def __init__(
        ...
        *,
        ...
        tp_group: Optional[paddle.distributed.ProcessGroup] = None,
    ):
...
def linear_with_frozen_weight(
    ...
    tp_group: Optional[paddle.core.ProcessGroup],
    grad_output_buffer: Optional[List[paddle.Tensor]] = None,
...
class LinearWithGradAccumulationAndAsyncCommunication(paddle.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.sequence_parallel:
            ...
            # reduce_scatter
            handle = dist.communication.stream.reduce_scatter._reduce_scatter_base(
        ...
        if ctx.gradient_accumulation_fusion:
            if wgrad_compute:
                ...
                elif weight.main_grad.dtype in (paddle.float16, paddle.bfloat16):
...
def linear_with_grad_accumulation_and_async_allreduce(
    ...
    async_grad_allreduce: Optional[bool] = None,
    tp_group: Optional[paddle.core.ProcessGroup] = None,
) -> paddle.Tensor:
...
class ColumnParallelLinear(paddle.nn.Linear):
    ...
    def __init__(
        ...
        *,
        ...
        embedding_activation_buffer: Optional[List[paddle.Tensor]] = None,
        grad_output_buffer: Optional[List[paddle.Tensor]] = None,
        ...
        if bias:
            if config.use_cpu_initialization:
                self.bias = Parameter(
                    paddle.empty([self.output_size_per_partition], dtype=config.params_dtype)
                )
            else:
                self.bias = Parameter(
                    paddle.empty(
        ...
        if config.gradient_accumulation_fusion and not _grad_accum_fusion_available:
            raise RuntimeError(
                ...
                "module is not found. You must turn off gradient accumulation fusion."
            )
...
class RowParallelLinear(paddle.nn.Linear):
    ...
    def __init__(
        ...
        *,
        ...
        if bias:
            if config.use_cpu_initialization:
                self.bias = Parameter(paddle.empty([self.output_size], dtype=config.params_dtype))
            else:
                self.bias = Parameter(
                    paddle.empty(
                        [self.output_size],
