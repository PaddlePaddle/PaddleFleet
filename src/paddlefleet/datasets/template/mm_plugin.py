...
from paddlefleet.transformers.qwen2_vl.vision_process import (
    fetch_image,
    fetch_video,
)
from paddlefleet.transformers.qwen3_omni_moe.processor import (
...
class KimiK3Plugin(BasePlugin):
    ...
    def process_messages(
        ...
        if self.image_token is None:
            raise ValueError(
                "KimiK3Plugin requires image_token to be set (e.g., '<|media_pad|>')."
            )
        ...
        if self.expand_mm_tokens:
            ...
            merge_kernel_size = int(
                image_processor.media_proc_cfg["merge_kernel_size"]
            )
        else:
        ...
        for msg in messages:
            ...
            while IMAGE_PLACEHOLDER in content:
                ...
                if self.expand_mm_tokens:
                    _, grid_h, grid_w = (
                        int(x) for x in image_grid_thw[num_image_tokens]
                    )
                    image_seqlen = (grid_h // merge_kernel_size) * (
                        grid_w // merge_kernel_size
                    )
                else:
                    image_seqlen = 1
                repl = (
