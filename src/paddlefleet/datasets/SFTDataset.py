...
from paddlefleet.datasets.reader.mix_datasets import create_dataset_instance
from paddlefleet.datasets.reader.multi_source_datasets import MultiSourceDataset
from paddlefleet.transformers.tokenizer_utils import PretrainedTokenizer
from paddlefleet.utils.env import NONE_CHAT_TEMPLATE
        ...
        while True:
            try:
                ...
                try:
                    ...
                    print(
                        f"Warning: Error processing example in worker, skipping. Error: {str(e)}"
                    )
        ...
        if self.dataset_num_proc > 1:
            ...
            for raw_idx in range(len(self.mix_datasets)):
                ...
                try:
                    ...
                    print(
                        f"Warning: Error processing example, skipping. Error: {str(e)}"
                    )
                    result = None
        ...
        if self.use_template:
            if self.template_backend == "jinja":
                ...
                if (
                    objects.get("bbox")
                    and images
                    and self.template.grounding_plugin.norm_bbox != "none"
                ):
