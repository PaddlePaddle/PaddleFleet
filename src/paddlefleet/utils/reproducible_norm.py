...
class ReproducibleL2Norm:
    ...
    def __init__(
        self, place: paddle.base.libpaddle.Place | None = None
    ) -> None:
        self.place = place
    ...
    def add(
        self, bins: paddle.Tensor, indices: paddle.Tensor, values: paddle.Tensor
    ) -> paddle.Tensor:
    ...
    def accumulate(
        ...
        if flat.shape[0] > 2**40:
            raise OverflowError(
                "Reproducible norm supports at most 2**40 global elements"
            )
        bins = self.add(
            bins,
            self.tensor([22], "int64"),
            self.tensor([flat.shape[0]], "int64"),
        )
        for start in range(0, flat.shape[0], chunk_size):
            ...
            mantissa = fraction | paddle.where(
                exponent > 0,
                paddle.full_like(exponent, 0x800000),
                paddle.zeros_like(exponent),
            )
            ...
            for offset in range(3):
                bins = self.add(
                    bins,
                    limb + offset,
                    (shifted >> (16 * offset)) & self.tensor(65535, "int64"),
                )
            flags = paddle.stack(
                [
                    self.cast(
                        (exponent == 255) & (fraction == 0), "int64"
                    ).sum(),
                    self.cast(
                        (exponent == 255) & (fraction != 0), "int64"
                    ).sum(),
    ...
    def finish(
        ...
        top = paddle.where(
            digits != 0, index, paddle.full_like(index, -1)
        ).max()

        def get(i):
            return paddle.where(
                index == i, digits, paddle.zeros_like(digits)
            ).sum()
        ...
        round_bit = (
            (round_word >> round_shift) & self.tensor(1, "int64")
        ) * self.cast(cut > 0, "int64")
        sticky = (
            paddle.where(
                index < round_limb, digits, paddle.zeros_like(digits)
            ).sum()
            != 0
        )
        sticky = sticky | (
            (round_word & ((self.tensor(1, "int64") << round_shift) - 1)) != 0
        )
        ...
        raw = paddle.where(
            exponent >= 255, paddle.full_like(raw, 0x7F800000), raw
        )
        raw = paddle.where(highest < 23, get(0) | (get(1) << 16), raw)
        raw = paddle.where(top < 0, paddle.zeros_like(raw), raw)
        raw = paddle.where(
            bins[20] != 0, paddle.full_like(raw, 0x7F800000), raw
        )
        raw = paddle.where(
            bins[21] != 0, paddle.full_like(raw, 0x7FC00000), raw
        )
...
class ReproducibleClipGradByGlobalNorm(paddle.nn.ClipGradByGlobalNorm):
    ...
    def _scale_grads(self, params_grads, norm):
        coefficient = paddle.minimum(
            self.clip_norm / (norm + 1e-6), paddle.ones_like(norm)
        )
        for parameter, gradient in params_grads:
            if gradient is not None and getattr(parameter, "need_clip", True):
...
def restore_reproducible_clip(dist_optimizer) -> bool:
    ...
    def rewrap(wrapper):
        if type(wrapper) is not HybridParallelClipGrad:
            return None
        inner = wrapper
        while inner is not None and not isinstance(
            inner, ReproducibleClipGradByGlobalNorm
        ):
            inner = getattr(inner, "_clip", None)
        if inner is None:
            return None
        return ReproducibleHybridParallelClipGrad(
            inner, wrapper._hcg, wrapper.split_norm_comm, wrapper._timers
        )
