...
"""
This module is used to store environmental variables in PaddleFleet.
PF_HOME              -->  the root directory for storing PaddleFleet related data. Default to ~/.paddlefleet. Users can change the
    ...
    if "PF_HOME" in os.environ:
        ...
        if os.path.exists(home_path):
            if os.path.isdir(home_path):
                ...
                raise RuntimeError(
                    "The environment variable PF_HOME {} is not a directory.".format(
                        home_path
                    )
                )
        else:
            return home_path
    return os.path.join(_get_user_home(), ".paddlefleet")
