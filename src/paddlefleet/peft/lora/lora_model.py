...
def get_lora_layers():
    try:
        if get_env_device() == "xpu":
            # If paddle_xpu is not installed, just use PaddleFleet's native lora layers
...
class LoRAModel(nn.Layer):
    ...
    def _find_and_replace_module(self, model, module_name, lora_config):
        ...
        if isinstance(module, nn.Linear) or isinstance(module, FusedLinear):
            ...
            isinstance(module, FleetColumnParallelLinear)
            or isinstance(module, FleetRowParallelLinear)
        ):
        ...
        elif is_paddlefleet_available() and isinstance(
        ...
        elif isinstance(module, MoeExpertsBase) or isinstance(
            module, GroupedMLPExpert
        ):
        ...
        if lora_module is None:
            raise ValueError(
                f"LoRA strategy only supports paddle.nn.Linear or paddle.distributed.fleet.meta_parallel.ColumnParallelLinear or paddlefleet.transformers.sequence_utils. {module}({module_name} {type(module).__name__}) is not supported。"
            )
    ...
    def mark_only_lora_as_trainable(self) -> None:
        for _, layer in self.model.named_sublayers():
            if (
                ...
                or (
                    QuantizationLoRALinear is not None
    ...
    def get_lora_model(
        self, model: Union[PretrainedModel, nn.Layer], lora_config: LoRAConfig
    ):
