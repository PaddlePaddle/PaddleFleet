...
def _deterministic_attn_sink_enabled():
    return os.getenv("DSV4_TILELANG_SPARSE_MLA_DETERMINISTIC_ATTN_SINK", "0").lower() in {"1", "true", "yes", "on"}


def _deterministic_dkv_enabled():
    return os.getenv("DSV4_TILELANG_SPARSE_MLA_DETERMINISTIC_DKV", "0").lower() in {"1", "true", "yes", "on"}
