...
from paddlefleet.transformers import (  # ChatGLMv2Tokenizer,
    AutoTokenizer,
    PretrainedConfig,
)
...
def get_infer_model_path(input_dir, model_prefix):
    if dist.get_world_size() > 1:
        ...
        return os.path.join(
            input_dir, "rank_{}".format(local_rank), model_prefix
        )
    else:
...
def get_rotary_position_embedding(
    position_ids, head_dim, rope_theta=10000.0, rope_scaling: dict = None
):
    """
