...
def _reexec_with_numactl(local_rank, numa_node):
    ...
    print(
        f"[PaddleFleet] Re-exec trainer with numactl: local_rank={local_rank}, numa_node={numa_node}",
        flush=True,
    )
...
def _maybe_bind_trainer_numa():
    ...
    print(
        f"[PaddleFleet] Trainer NUMA affinity enabled: local_rank={local_rank}, "
...
def launch():
    ...
    if command == "train":
        ...
        from paddlefleet.cli.train.tuner import run_tuner
