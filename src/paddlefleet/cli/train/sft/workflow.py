...
from paddlefleet.data.indexed_dataset import SFTMMapIndexedDatasetBuilder
from paddlefleet.datasets.collate import collate_fn, mm_collate_fn
from paddlefleet.datasets.data_utils import estimate_training
from paddlefleet.datasets.loader import create_dataset as create_dataset_sft
from paddlefleet.datasets.loader import create_indexed_dataset
...
from paddlefleet.peft import LoRAConfig, LoRAModel
...
def project_owning_loader_semantics(
    input_values, model_label_values, position_values=None
):
    ...
    if (
        semantic_length <= 0
        or semantic_length > len(input_values)
        or semantic_length > len(model_label_values)
    ):
        raise ValueError(
    ...
    semantic_label_values = (
        semantic_model_label_values[-1:] + semantic_model_label_values[:-1]
    )
...
class ModelReproObservationCallback(TrainerCallback):
    ...
    def _forward_contract_specs(cls, boundary_set):
        ...
        raise ValueError(
            f"unsupported MODEL_REPRO_FORWARD_BOUNDARY_SET: {boundary_set}"
        )
    ...
    def _normalized_device():
        ...
        return (
            "cuda"
            if paddle.device.get_device().split(":")[0] in ("gpu", "cuda")
            else "cpu"
        )
    ...
    def on_train_end(self, args, state, control, **kwargs):
        if not self.loss_path or not self._is_writer(state):
            return
        raw_path = (
            Path(self.raw_loss_path).expanduser().resolve()
            if self.raw_loss_path
            else None
        )
        payload = self._machine_loss_payload(
            self._loss_events,
            raw_path=raw_path,
            source_sha256=self._sha256_file(raw_path)
    ...
    def _source_modules():
        names = (
            "paddlefleet.cli.train.sft.workflow",
            "paddlefleet.trainer.trainer",
    ...
    def _topology(self, args):
        ...
        record = {
            "configured": {
                name: getattr(self.model_config, name, None) for name in fields
            },
            "world_size": paddle.distributed.get_world_size(),
    ...
    def _parameter_record(param):
        ...
        if bit_dtype is not None:
            ...
            sign_bit = np.array(
                1 << (np.dtype(bit_dtype).itemsize * 8 - 1), dtype=bit_dtype
            )
        ...
        if tensor.ndim == 2:
            record["transpose_sha256"] = hashlib.sha256(
                np.ascontiguousarray(array.T).tobytes()
            ).hexdigest()
        return record
    ...
    def _write_forward_record(self, boundary, value):
        ...
        call_index = sum(
            name == boundary or name.startswith(f"{boundary}_call")
            for name in records
        )
        ...
        payload = {
            ...
            "boundary_set": getattr(
                self, "_forward_contract_boundary_set", "coarse"
            ),
            "selectors": getattr(
                self, "_forward_contract_selector_receipt", []
            ),
            "records": records,
        }
        with open(
            os.path.join(rank_dir, "metadata.json"), "w", encoding="utf-8"
        ) as stream:
            json.dump(
    ...
    def _install_forward_contract_once(self, model):
        ...
        if (
            not output_dir
            or getattr(self, "_forward_contract_installed", False)
            or model is None
        ):
            return
        rank = (
        ...
        boundary_set = os.environ.get(
            "MODEL_REPRO_FORWARD_BOUNDARY_SET", "coarse"
        )
        ...
        if fine_specs is not None:
            ...
            if rank < 2:
                ...
                invalid = {
                    name: len(hits)
                    for name, hits in module_hits.items()
                    if len(hits) != 1
                }
                if invalid:
                    raise RuntimeError(
                ...
                for module_name, boundary in fine_specs.items():
                    ...
                    handles.append(
                        module.register_forward_post_hook(
                            ...
                            _inputs,
                            output,
                            name=boundary: self._write_forward_record(
                                name, output
                            )
                        )
                    )
                    selected.append(
                        {"module": module_name, "boundary": boundary}
                    )
        ...
        for module_name, module in model.named_sublayers():
            ...
            if module_name == "0.embedding":
                ...
                handles.append(
                    module.register_forward_pre_hook(
                        lambda _module,
                        inputs,
                        name=input_boundary: self._write_forward_record(
                            name, inputs
                        )
                    )
                )
                ...
                handles.append(
                    module.register_forward_pre_hook(
                        lambda _module,
                        inputs,
                        name="output_head_input": self._write_forward_record(
                            name, inputs
                        )
                    )
                )
            ...
            elif module_name.startswith("6.") and module_name.rsplit(".", 1)[
                -1
            ] in {
                "enorm",
                "hnorm",
            ...
            if boundary is not None:
                handles.append(
                    module.register_forward_post_hook(
                        ...
                        _inputs,
                        output,
                        name=boundary: self._write_forward_record(name, output)
                    )
                )
        self._forward_contract_handles = handles
        self._forward_contract_installed = True
    ...
    def _write_parameter_contract_once(self, model):
        if (
            not self.parameter_receipt_dir
            or self._parameters_written
            or model is None
        ):
            return
        rank = (
    ...
    def on_load_data_end(self, args, state, control, inputs=None, **kwargs):
        ...
        if (
            not self.input_receipt_path
            or self._input_written
            or not self._is_writer(state)
        ):
            return
        inputs = inputs or {}
        ...
        position_values = (
            self._values(position_ids) if position_ids is not None else None
        )
        ...
        has_mtp_sentinel = (
            ...
            and (
                ...
                or position_values[-mtp_depth:] == [0] * mtp_depth
            )
        )
        payload = {
            ...
            "carrier_padding": {
                ...
                "input_ids_sha256": self._digest(
                    input_values[len(semantic_input_values) :]
                ),
                "labels_sha256": self._digest(
                    label_values[len(semantic_input_values) :]
                ),
            },
            "mtp_sentinel": {
    ...
    def on_log(self, args, state, control, logs=None, raw_loss=None, **kwargs):
        if (
            raw_loss is None
            or not self._is_writer(state)
            or not (self.raw_loss_path or self.loss_path)
        ):
            return
        event = {"step": int(state.global_step), "loss": float(raw_loss)}
        for key, value in (logs or {}).items():
            normalized_key = key.replace(" ", "_")
            if normalized_key.startswith("mtp_") and normalized_key.endswith(
                "_loss"
            ):
...
from paddlefleet.cli.hparams import (
    DataArguments,
    FinetuningArguments,
    GeneratingArguments,
    ModelArguments,
...
from paddlefleet.cli.hparams.preprocess_args import End2EndProcessorArguments
from paddlefleet.cli.utils import (
...
def validate_pretokenized_offline_dataset(dataset, expected_length):
    if dataset is None or len(dataset) == 0:
        raise ValueError(
            "pretokenized offline dataset must contain at least one row"
        )
    for row in dataset:
        if not isinstance(row, list) or len(row) != 1:
            raise ValueError(
                "pretokenized offline rows must contain exactly one TextSequence"
            )
        sequence = row[0]
        fields = {
...
def apply_glm_moe_dsa_training_contract(
    ...
    requested_mtp_loss_scaling_factor = getattr(
        training_args, "mtp_loss_scaling_factor", None
    )
    if requested_mtp_loss_scaling_factor is not None:
        model_config.mtp_loss_scaling_factor = float(
            requested_mtp_loss_scaling_factor
        )
    logger.info(
    ...
    if (
        getattr(data_args, "pretokenized_dataset", False)
        and mtp_depth > 0
        and not model_args.mtp_attention_flexible
    ):
        raise ValueError(
    ...
    model_config.sequence_parallel = bool(
        getattr(training_args, "sequence_parallel", False)
    )
    ...
    expert_tensor_parallel_size = (
        1
        if configured_expert_tensor_parallel_size == -1
        else configured_expert_tensor_parallel_size
    )
...
def freeze_param_except_mtp(model, config):
    ...
    mtp_depth = int(
        ...
        or getattr(config, "mtp_num_layers", 0)
        or 0
    )
    jackpot = set(
        range(config.num_hidden_layers, config.num_hidden_layers + mtp_depth)
    )
...
def create_pretrained_dataset(training_args, data_args, model_args):
    ...
    from paddlefleet.data import Stack
    ...
    def _collate_data(batch, stack_fn=Stack()):
        ...
        for batch_sequence in batch:
            ...
            padded_position_ids = np.array(
                [sum(batch_sequence["position_ids"], [])[:-1]]
            )
            return_list.append(
                [
                    padded_token_ids,
...
def run_sft(
    ...
    if (
        getattr(model_config, "model_type", None) != "glm_moe_dsa"
        and getattr(model_config, "mtp_num_layers", 0) > 1
    ):
    ...
    if getattr(model_config, "text_config", None) is not None:
        ...
        model_config.text_config._attn_implementation = (
    ...
    if getattr(model_config, "vision_config", None) is not None:
        ...
        model_config.vision_config.recompute_modules = getattr(
            model_config, "recompute_modules", None
        )
    ...
    if data_args.template == "glm5_2":
        dataset_config["enable_thinking"] = getattr(
            generating_args, "enable_thinking", None
        )
    ...
    if data_args.dataset_type == "pretrain":
        ...
        if data_args.pretokenized_dataset:
            validate_pretokenized_offline_dataset(
                train_dataset, data_args.max_seq_len
            )
            if (
                training_args.num_nextn_predict_layers > 0
                and data_args.pretokenized_pad_token_id is None
            ):
                raise ValueError(
                    "pretokenized_pad_token_id is required when MTP padding is enabled"
                )
            logger.info(
                "Using validated pretokenized offline dataset without text tokenization."
            )
    ...
    if getattr(model, "is_fleet", False) and not model_args.lora:
        ...
        logger.info(
            f"Setting max_seq_len to {max_seq_len} using PaddleFleet Model."
        )
    if data_args.dataset_type != "pretrain":
        if "VL" in model_args.stage:
            ...
            data_collator = partial(
                ...
                padding_free=data_args.padding_free,
                input_pad_token_id=(
                    data_args.pretokenized_pad_token_id
                    if data_args.pretokenized_dataset
                    else None
                ),
            )
    ...
    if (
        getattr(model_config.get_text_config(), "topk_method", None)
        == "noaux_tc"
    ):
        callbacks += [
            MoECorrectionBiasAdjustCallback(
