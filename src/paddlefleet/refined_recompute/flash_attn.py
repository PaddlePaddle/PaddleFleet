...
class FlashMaskAttnFunctor(PyLayer):
    ...
    @staticmethod
    def forward(ctx, q, k, v, startend_row_indices, learnable_sink, hold_tensors):
        """
    ...
    def backward(ctx, grad):
        ...
        if fa_version == 2:
            ...
            if startend_row_indices is not None:
                ...
                flashmask_info = None
            q_grad, k_grad, v_grad, grad_sink = _flash_attn_bwd(
                q.detach(),
                k.detach(),
                v.detach(),
                result_attention,
        ...
        # must be None -- grad_sink belongs in position 4. A fixed off-by-one
        # sink is also stop_gradient=True, so for it the 3-tuple is correct.
        if ctx.sink_requires_grad:
            return q_grad, k_grad, v_grad, None, grad_sink
        return q_grad, k_grad, v_grad
