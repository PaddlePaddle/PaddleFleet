...
class DistDataLoader(paddle.io.DataLoader):
    ...
    def __len__(self):
        if self._need_data:
            ...
            raise ValueError(
                "raise error for `paddlefleet.trainer.trainer_utils.has_length`"
            )
...
def init_stream_data_group():
    ...
    for rank in range(world_size):
        ...
        logger.info(
            "[dataflow] rank: {}, model: {}, pipe: {}, sep: {}".format(
                rank, coord.model, coord.pipe, coord.sep
            )
        )
        if (
            coord.model == 0
    ...
    def __init__(
        ...
        if self._is_global_rank_0:
            ...
            logger.info(
                ...
                "only global rank 0 reads.".format(
