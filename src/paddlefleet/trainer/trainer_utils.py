...
def set_random_seed(
    ...
    if seed_ is not None and seed_ > 0:
        ...
        if is_paddlefleet_available():
            ...
            seed = seed_ + (
                100
                * paddlefleet.parallel_state.get_pipeline_model_parallel_rank()
            )
...
def _restore_master_weights_2d_on_device(master_weights, group, param_sink):
    ...
    if not reshard_util.use_device_gather():
        return None
    return reshard_util.all_gather_on_device(
        master_weights, group, param_sink=param_sink
    )
...
class EMAStateAssembler:
    ...
    def _build_ema_sharded_state_dict(self, ema_state_dict):
        ...
        if hasattr(self.model, "_hf_flatten_sharded_state_dict"):
            ema_sharded_state_dict = self.model._hf_flatten_sharded_state_dict(
                ema_sharded_state_dict
            )
