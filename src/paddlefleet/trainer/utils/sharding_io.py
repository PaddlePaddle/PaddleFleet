...
from ...transformers.utils import paddlefleet_load
...
def exclude_parameters_in_state_dict(
    ...
    ), "param_names_in_master_weights type:{}".format(
    ...
    logger.info(
        "sharding_group_param_names:{}".format(param_names_in_master_weights)
    )
...
class ParameterNameRemapper:
    def __init__(self, old_mapping, new_mapping, checkpoint):
        ...
        self.new_mapping = dict(
            [(k, new_mapping[k]) for k in old_mapping.keys()]
        )
...
class ShardingIO:
    def __init__(
        ...
        reshard_util.set_broadcast_max_chunk_bytes(
            int(args.reshard_bucketed_broadcast_max_chunk_gb * (1024**3))
        )
        self.model = model
    ...
    def _load_optimizer_state_of_one_shard(
        ...
        if os.path.isfile(path):
            opt_state = paddlefleet_load(path, map_location="cpu")
            if self.is_ema:
                opt_state = {
    ...
    def _recover_params_from_master_weights(
        ...
        logger.info(
            "before recover, model_state_dict number: {}".format(
        ...
        logger.info(
            "after recover, casted model_state_dict number: {}".format(
                len(model_state_dict)
            )
        )
