...
def get_fused_param_mappings(
    optimizer, manipulated_state_dict, staging_holder=None
):
    param_mappings = {}
    ipc_meta_mappings = {}
    index = 0
...
def pin2cpu_zero_copy_fp32(t):
    ...
    arr = np.ctypeslib.as_array(
        ctypes.cast(lt._ptr(), ctypes.POINTER(ctypes.c_float)),
        shape=(t._numel(),),
    )
    ...
    def ema_accumulate(self, global_step, loss, zcc_ema_loss_threshold):
        ...
        with device_guard("cpu"):
            ...
            if (
                ...
                cpu_master_weights = cpu_buffer._slice(
                    self.master_min_offset, self.master_max_offset
                ).cpu()
        ...
        if self.args.bf16:
            ...
            logger.info(
                "param_names_in_master_weights len:{}, bf16 state_dict len:{}, :{}".format(
                    len(param_names_in_master_weights),
                    len(state_dict),
                    state_dict.keys(),
                )
            )
            ...
            if self.grouped_gemm_params and len(self.grouped_gemm_params) > 0:
                for k, v in opt_state_dict.items():
                    ...
                    if (
                        ...
                        tgt = _zcc_lookup_local_shape(
                            self.opt_ckpt_meta, k, int(v.numel())
                        )
                        opt_state_dict[k] = (
                            v.reshape(tgt)
                            if tgt
                ...
                for k, v in master_weights.items():
                    ...
                    if (
                        ...
                        origin_shape = v.shape
                        tgt = _zcc_lookup_local_shape(
                            self.master_weight_ckpt_meta, k, int(v.numel())
                        )
                        master_weights[k] = (
                            v.reshape(tgt)
                            if tgt
            ...
            if self.grouped_gemm_params and len(self.grouped_gemm_params) > 0:
                for k, v in list(master_weights.items()):
                    ...
                    if (
                        ...
                        origin_shape = v.shape
                        tgt = _zcc_lookup_local_shape(
                            self.ema_master_weight_ckpt_meta, k, int(v.numel())
                        )
                        master_weights[k] = (
                            v.reshape(tgt)
                            if tgt
