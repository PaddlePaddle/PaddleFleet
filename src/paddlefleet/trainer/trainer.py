...
from paddle.distributed.fsdp.fully_shard import (
...
class Trainer:
    ...
    def __init__(
        ...
        reshard_util.set_broadcast_max_chunk_bytes(
            int(
                getattr(
                    self.args, "reshard_bucketed_broadcast_max_chunk_gb", 2.0
                )
                * (1024**3)
            )
        )
        reshard_util.set_device_gather(
            getattr(self.args, "reshard_master_weight_device_gather", False)
        )
        ...
        if _im_monitors and _im_interval > 0:
            ...
            default_callbacks.append(
                InternalMedicineCallback(
                    ...
                    debug_mode=getattr(
                        self.args, "internal_medicine_debug_mode", False
                    ),
                    log_dir=_im_log_dir,
                )
            )
        ...
        if self.args.pipeline_model_parallel_size > 1 or (
            ...
            if self.amp_dtype == "float16" or self.amp_dtype == "bfloat16":
                if (
                    ...
                    or ShardingOption.FSDP in self.args.sharding
                ):
                    if self.args.amp_master_grad:
                        mix_precision_utils.MixPrecisionScaler(
                            self.scaler
    ...
    def _wait_flex_async_save(self):
        ...
        """
        if (
            hasattr(self, "_flex_async_saver")
            and self._flex_async_saver.is_saving
        ):
    ...
    def _load_flex_checkpoint(self, resume_from_checkpoint):
        ...
        if self.args.load_from_hf:
            ...
            if hf_checkpoint_is_quantized(resume_from_checkpoint):
                gen_hf_quan_config = getattr(
                    self.model, "_gen_hf_quan_config", None
                )
            ...
            load_transform_kwargs = (
                ...
                else {"load_transform": load_transform}
            )
            dist.load_state_dict(
                model_sharded_state_dict,
                resume_from_checkpoint,
        ...
        if not self.args.sharded_model_from_ema:
            ...
            if os.getenv("HACK_CONVERT_CKPT", "0").lower() in [
                "true",
                "1",
            ] and os.getenv("HACK_CONVERT_CKPT_NOPP_TO_PP", "0").lower() in [
                "true",
                "1",
            ]:
                logger.info(
                    "[AOAConfig] generate master_weight_aoa by _gen_ckpt_convert_aoa !"
                )
                master_weight_aoa = self.model._gen_ckpt_convert_aoa(
                    self.model.config
                )
            ...
            with _sprof_span(
                "load_master_weight", collective=True, n=len(master_weights)
            ):
                dist.load_state_dict(
                    master_weights,
                    master_weights_path,
            ...
            if not self.args.ignore_load_lr_and_optim:
                ...
                if os.getenv("HACK_CONVERT_CKPT", "0").lower() in [
                    ...
                    logger.info(
                        "[AOAConfig] generate opt_stat_aoa by _gen_ckpt_convert_aoa !"
                    )
                    opt_stat_aoa = self.model._gen_ckpt_convert_aoa(
                        self.model.config, target="opt_state"
                    )

                with _sprof_span(
                    "load_opt_state", collective=True, n=len(opt_states)
                ):
                    dist.load_state_dict(
                        opt_states,
                        opt_states_path,
        ...
        if self.args.sharded_model_from_ema:
            ...
            def bf16_filtered_sharded_state_dict(sharded_state_dict):
                ...
                for k, v in sharded_state_dict.items():
                    if (
                        v.local_tensor.dtype == paddle.bfloat16
                        and not v.local_tensor.stop_gradient
                    ):
                        continue
                    new_state_dict[k] = v
            ...
            if os.getenv("HACK_CONVERT_CKPT", "0").lower() in ["true", "1"]:
                ...
                if os.getenv("HACK_CONVERT_CKPT_NOPP_TO_PP", "0").lower() in [
                    "true",
                    "1",
                ]:
                    logger.info(
                        "[AOAConfig] generate model_state_aoa by _gen_ckpt_convert_aoa_model_state !"
                    )
                    aoa_config = self.model._gen_ckpt_convert_aoa_model_state(
                        self.model.config
                    )
            ...
            with _sprof_span(
                "load_model_state",
                collective=True,
                n=len(model_sharded_state_dict),
            ):
                dist.load_state_dict(
                    model_sharded_state_dict,
                    model_states_path,
        ...
        if enable_bf16_opt:
            ...
            def _assign_master_weights_to_model(master_weights):
                ...
                for key, param in model_state_dict.items():
                    if (
                        ...
                        assert param.shape == value.shape, (
                            f"got {param.shape} vs {value.shape}"
                        )
            ...
            def recover_params_from_master_weight(opt_state_dict, group):
                ...
                param_2d_names = (
                    _get_muon_2d_param_names(muon_opt)
                ...
                with _sprof_span(
                    "mw_cast_bf16", n=len(tmp), n_2d=len(param_2d_names)
                ):
                    for k, v in tmp.items():
                        name = v.name
                        if (
                            k in param_2d_names
                            and reshard_util.use_device_gather()
                        ):
                ...
                if muon_opt is not None:
                    ...
                    with _sprof_span(
                        "restore_2d", collective=True, n=len(mw_2d)
                    ):
                        restored_2d = _restore_master_weights_2d_on_device(
                            mw_2d, group, device_param_sink
                        )
                        if (
                            restored_2d is None
                        ):  # reshard_master_weight_device_gather=False
                            with _sprof_span(
                                "restore_2d_host", collective=True
                            ):
                    ...
                    with _sprof_span(
                        "restore_1d", collective=True, n=len(mw_1d)
                    ):
                    ...
                    with _sprof_span(
                        "restore_master_weights",
                        collective=True,
                        n=len(master_weights),
                    ):
            ...
            with (
                _sprof_span("master_weight_writeback", collective=True),
                paddle.no_grad(),
            ):
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                # for paddlefleet.utils.batch_sampler.(NlpDistributedBatchSampler & MappingBatchSampler & MappingDistributedBatchSampler)
    ...
    def _get_eval_sampler(self, eval_dataset: Dataset):
        ...
        if self.args.world_size <= 1:
            ...
            if (
                is_paddlefleet_available() and self.using_fleet_model
            ) or self.args.pipeline_model_parallel_size > 1:
    ...
    def _build_muon_param_info_map(self):
        ...
        for pp_name, param in model.named_parameters():
            use_muon = _default_should_use_muon(
                pp_name, param.shape, exclude_patterns
        ...
        if self.optimizer is None:
            ...
            if use_accuracy_compatible:
                ...
                if (
                    ...
                    decay_parameters = {
                        ...
                        and not any(
                            pattern.search(n.lower()) for pattern in forbidden
                        )
                    }
            ...
            if self.args.optim == OptimizerNames.MUON:
                ...
                if hasattr(self.model, "build_muon_param_info_map"):
                    ...
                    optimizer_kwargs["muon_param_info_map"] = (
                        self._build_muon_param_info_map()
                    )
                logger.info(
    ...
    def _build_grad_clip(self):
        ...
        if hf_bitexact_clip_enabled(accuracy_target):
            ...
            return HFBitexactClipGradByGlobalNorm(
                self.args.max_grad_norm, trainer=self
            )
    ...
    def get_optimizer_cls_and_kwargs(
        ...
        Args:
            args (`paddlefleet.training_args.TrainingArguments`):
    ...
    def _wrap_model(self, model, training=True):
        ...
        if (
            in_fsdp_mode
            and not self.args.amp_master_grad
            and self.amp_dtype in ("float16", "bfloat16")
        ):
            fsdp_mp_policy = MixedPrecisionPolicy(
                reduce_dtype=getattr(paddle, self.amp_dtype)
            )
    ...
    def _prepare_input(
        self, data: Union[paddle.Tensor, Any]
    ) -> Union[paddle.Tensor, Any]:
        """
    ...
    def training_pipeline_step(
        ...
        with (
            ...
            _sprof_span("pp_forward_backward", collective=True),
        ):
    ...
    def print_config(self, args=None, key=""):
        ...
        logger.debug(
            "{:30}: {}".format(
                "paddlefleet commit id", paddlefleet.version.commit
            )
        )
        if is_paddlefleet_available():
            import paddlefleet
