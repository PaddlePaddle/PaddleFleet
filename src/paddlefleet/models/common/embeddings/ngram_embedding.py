...
The tables and projections are not tensor-parallel sharded, so this is written for TP = 1.
...
class NgramEmbedding(nn.Layer):
    ...
    Given input_ids [B, S], computes N-gram hash IDs for each (n, k) pair, looks up sub-embeddings, projects to hidden_size, and returns the aggregated N-gram signal to be added to the standard word embeddings.
    ...
    def __init__(self, config: TransformerConfig, vocab_size: int):
        ...
        assert self.n >= 2 and self.k >= 1, (
            "ngram_emb_neighbor_num must be >= 2 and ngram_emb_split_num >= 1, "
            f"got {self.n} and {self.k}"
        )
    ...
    def _compute_ngram_hash_ids(
        ...
        for k_offset in range(2, ngram + 1):
            ngram_ids = (
                ngram_ids
                + shifted_ids[k_offset].cast("int64") * mods[k_offset - 2]
            )
    ...
    def forward(self, input_ids: Tensor) -> Tensor:
        ...
        Returns:
            [B, S, H] N-gram embedding signal, to be added to the word embeddings and divided by normalizer together.
        """
        vocab_mods = self._precompute_vocab_mods()
        shifted_ids = self._compute_shifted_ids(input_ids)

        # Accumulated from the first projection instead of from a zeros of the
        # table dtype, which under AMP is not the projection's output dtype.
        ngram_output = None
