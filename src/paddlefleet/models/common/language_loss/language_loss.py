...
class LanguageLoss(FleetLayer):
    ...
    def __init__(
        ...
        # Only consult the recompute plan when the config carries a real one:
        # module_needs_recompute rejects anything that is not a sequence or
        # dict, and test doubles hand us bare mocks here.
        _recompute_modules = getattr(config, "recompute_modules", None)
        if (
            self.defer_token_normalization is True
            and isinstance(_recompute_modules, (list, tuple, set, dict))
            and module_needs_recompute("loss_fn", None, config) is True
        ):
            # The deferred divisor lives on the instance and is cleared in a
            # `finally` before backward runs, so a recomputed loss_fn would
            # re-enter forward_impl with it already gone and silently scale
            # the MTP head like the main head. Refuse rather than mis-scale.
            raise NotImplementedError(
                "defer_token_normalization cannot be combined with loss_fn "
                "recompute: the deferred token divisor is lost when the loss "
                "is recomputed in backward. Disable one of the two."
            )
    ...
    def forward(self, logits: Tensor | list, labels: Tensor) -> Tensor:
        if isinstance(logits, list):
            ...
            def add_loss(main_loss, loss):
                accuracy_compatible = getattr(
                    self, "use_accuracy_compatible", False
                )
                if _use_accuracy_compatible_kernel() or accuracy_compatible:
...
class MainLanguageLoss(LanguageLoss):
    ...
    def forward(self, dict_args: dict | list, labels: Tensor) -> Tensor:
        ...
        def add_loss(main_loss, loss):
            if _use_accuracy_compatible_kernel() or getattr(
                self, "use_accuracy_compatible", False
            ):
                # Megatron-aligned: MTP loss gradient flows but loss scalar unchanged.
