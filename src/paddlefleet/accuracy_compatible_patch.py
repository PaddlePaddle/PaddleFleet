...
# =============================================================================
# Trainer / data-pipeline helpers
#
# These mirror the trainer-side half of the DSV4 accuracy-compatible replay.
# Every one of them is only reachable from a ``use_dsv4_accuracy_compatible()``
# guarded call site (or from ``LOAD_FIXED_DATA_PATH``, which is a debug-only
# env var), so the default training path is untouched.
# =============================================================================
...
def _resolve_fixed_training_files(
    ...
    manifest_path = os.path.join(
        fixed_tokens_path, f"manifest_rank{rank}.jsonl"
    )
    ...
    matches = sorted(
        glob.glob(
            os.path.join(
                fixed_tokens_path,
                f"tokens_step{fixed_step}_micro{micro_step}"
                f"_rank{rank}_seq*.npy",
            )
        )
    )
    if not matches:
        matches = sorted(
            glob.glob(
                os.path.join(
                    fixed_tokens_path,
                    f"tokens_step{fixed_step}_rank{rank}_seq*.npy",
                )
            )
        )
    if len(matches) == 1:
        ...
        labels_file = os.path.join(
            fixed_tokens_path,
            os.path.basename(tokens_file).replace("tokens_", "labels_", 1),
        )
        return tokens_file, labels_file

    raise FileNotFoundError(
        "Unable to resolve fixed training data files for "
        f"rank={rank} step={fixed_step} micro={micro_step} "
        f"seq_len={seq_len} in {fixed_tokens_path}"
    )
...
def load_fixed_training_data(
    ...
    accumulation_steps = max(
        1, int(getattr(training_args, "gradient_accumulation_steps", 1))
    )
    ...
    if fixed_input_arr.ndim != 2:
        raise ValueError(
            "LOAD_FIXED_DATA_PATH expects 1D or 2D token arrays, "
            f"got shape {fixed_input_arr.shape}"
        )
    ...
    return FixedTrainingData(
        ...
        max_seq_len=calc_padding_size(
            max(len(row) for row in input_ids), training_args
        ),
        batched=True,
    )
...
def flush_sequence_first_wgrad(model) -> None:
    """Move the seq-first wgrads stashed on parameters into their real grads.

    ``linear_seqfirst_wgrad`` / ``CompatibleOGroupProjection`` accumulate the
    Megatron-ordered weight gradient on the parameter object instead of adding
    it into ``main_grad`` immediately. Called once per optimizer step, before
...
def has_optimizer_state(
    struct_name, state_dict_metadata, optimizer_state_names
):
    """Whether the checkpoint metadata carries optimizer state for a parameter.

    A pure predicate: the caller owns the ``use_dsv4_accuracy_compatible()`` decision
    about *when* this has to hold, so both the accuracy-compatible branch and the
    original ``HACK_CONVERT_CKPT`` branch can share it.
    """
    return any(
