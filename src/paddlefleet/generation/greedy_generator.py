...
def _collect_prompt_log_probs(
    ...
    # 逐位对齐用（env 门控，默认关）：训练侧 per-token CE 走的是
    # paddle.nn.CrossEntropyLoss(reduction="none")（language_loss.py:245，
    # 即 softmax_with_cross_entropy 一个 kernel 内出结果，只舍入一次）。
    # 这里默认的 log_softmax + take_along_axis 会先把整张 [B,S,V] 的 log-prob
        ...
        with paddle.amp.auto_cast(True, level="O2", dtype="bfloat16"):
            ...
            for step in range(max_new_tokens):
                ...
                if isinstance(logits, (list, tuple)):
                    # MTP（num_nextn_predict_layers>0）等附加头模型返回 logits 列表，取 main head（index 0）
                    logits = logits[0]
                if _log_this_step:
