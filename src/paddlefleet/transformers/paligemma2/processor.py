...
class PaliGemmaProcessor(ProcessorMixin):
    ...
    def __init__(
        ...
        self.image_seq_length = getattr(
            image_processor, "image_seq_length", 1024
        )
            ...
            tokenizer.add_special_tokens(
                {
                    "additional_special_tokens": [
                        AddedToken(IMAGE_TOKEN, normalized=False)
                    ]
                }
            )
        ...
        **kwargs: Unpack[PaliGemmaProcessorKwargs],
        ...
        for prompt, sample_images in zip(text, image_batches):
            if IMAGE_TOKEN in prompt:
                if prompt.count(IMAGE_TOKEN) != len(sample_images):
                    raise ValueError(
                        "The number of `<image>` tokens must match the number of images for each prompt."
                    )
                ...
                prompt = (
                    IMAGE_TOKEN * (self.image_seq_length * len(sample_images))
                    + self.tokenizer.bos_token
                    + prompt
                )
        ...
        if suffix is not None:
            ...
            labels = np.concatenate(
                [
                    labels[:, 1:],
                    np.full((labels.shape[0], 1), -100, dtype=labels.dtype),
                ],
                axis=1,
            )
            data["labels"] = labels
    ...
    def _normalize_images(images, batch_size):
        if is_valid_image(images):
            if batch_size != 1:
                raise ValueError(
                    "The number of images must match the number of prompts."
                )
            return [[images]]
        if not isinstance(images, (list, tuple)) or not images:
            raise ValueError(
                "`images` must be an image or a list containing one image per prompt."
            )
