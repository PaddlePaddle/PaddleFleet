...
from paddlefleet.transformers.auto.configuration import (
...
def getattribute_from_module(module, attr):
    ...
    paddlefleet_module = importlib.import_module("paddlefleet")

    if module != paddlefleet_module:
        try:
            return getattribute_from_module(paddlefleet_module, attr)
        except ValueError:
            raise ValueError(
                f"Could not find {attr} neither in {module} nor in {paddlefleet_module}!"
            )
    else:
        raise ValueError(f"Could not find {attr} in {paddlefleet_module}!")
