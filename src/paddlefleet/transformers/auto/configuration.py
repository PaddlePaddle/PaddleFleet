    ...
    def __getitem__(self, key):
        ...
        if module_name not in self._modules:
            self._modules[module_name] = importlib.import_module(
                f".{module_name}.configuration", "paddlefleet.transformers"
            )
        ...
        transformers_module = importlib.import_module("paddlefleet")
...
def resolve_minicpm4_1_model_type(config_dict, model_type_override=None):
    """Resolve upstream MiniCPM4.1 configs to the PaddleFleet registration key."""
...
class AutoConfig(PretrainedConfig):
    ...
    def _get_config_class_from_config(
        ...
        model_class = import_module(f"paddlefleet.transformers.{model_name}")

        # To make AutoConfig support loading config with custom model_class
        # which is not in paddlefleet.transformers. Using "model_type" to load
