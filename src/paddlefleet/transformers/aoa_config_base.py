...
class MoEAOAConfigGenerator:
    ...
    def _extract_params(cls, config: Any) -> MoEAOAConfigParams:
        ...
        return MoEAOAConfigParams(
            ...
            index_n_heads=getattr(
                config, "index_n_heads", getattr(config, "dsa_index_n_heads", 0)
            )
            or 0,
            indexer_types=getattr(
                config,
                "indexer_types",
                getattr(config, "dsa_indexer_types", None),
            ),
    ...
    def _indexer_type_for_layer(
        ...
        if params.num_hidden_layers and layer_idx >= params.num_hidden_layers:
            return "full"
        if params.indexer_types is not None and layer_idx < len(
