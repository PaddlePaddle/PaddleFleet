...
from ..utils.import_utils import direct_paddlefleet_import
...
paddlefleet_module = direct_paddlefleet_import(Path(__file__).parent)
...
class _LazyAutoProcessorMapping(dict):
    ...
    _MAPPING_NAMES = {
        "image_processor": (
            "paddlefleet.transformers.auto.image_processing",
            "AutoImageProcessor",
        ),
        "video_processor": (
            "paddlefleet.transformers.auto.video_processing",
            "AutoVideoProcessor",
        ),
        "feature_extractor": (
            "paddlefleet.transformers.auto.feature_extraction",
            "AutoFeatureExtractor",
        ),
        "tokenizer": (
            "paddlefleet.transformers.auto.tokenizer",
            "AutoTokenizer",
        ),
    }
...
class VideosKwargs(TypedDict, total=False):
    ...
    size: Annotated[
        Optional[Union[int, list[int], tuple[int, ...], dict[str, int]]],
        image_size_validator(),
    ]
    ...
    return_tensors: Annotated[
        Optional[Union[str, TensorType]], tensor_type_validator()
    ]
...
class PaddleProcessorMixin:
    ...
    def to_json_file(
        self, json_file_path: Union[str, os.PathLike], legacy_serialization=True
    ):
        """
    ...
    def get_possibly_dynamic_module(module_name):
        if hasattr(paddlefleet_module, module_name):
            return getattr(paddlefleet_module, module_name)
        lookup_locations = [
            paddlefleet_module.IMAGE_PROCESSOR_MAPPING,
            paddlefleet_module.VIDEO_PROCESSOR_MAPPING,
            paddlefleet_module.TOKENIZER_MAPPING,
        ...
        for lookup_location in lookup_locations:
            for custom_class in lookup_location._extra_content.values():
                if isinstance(custom_class, tuple):
                    ...
                    return custom_class
        raise ValueError(
            f"Could not find module {module_name} in `paddlefleet`."
        )
