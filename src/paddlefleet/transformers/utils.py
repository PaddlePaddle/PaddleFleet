...
if TYPE_CHECKING:
    from paddlefleet.transformers import PretrainedModel
...
def get_scale_by_dtype(
    dtype: str = None, return_positive: bool = True
) -> float:
...
def adapt_stale_fwd_patch(self, name, value):
    ...
    if name == "forward":
        ...
        if new_args:
            if self.__module__.startswith("paddlefleet"):
                warnings.warn(
                ...
                warnings.warn(
                    ...
                    f"by paddlefleet which seems have more arguments such as {new_args}. "
                    "We automatically add compatibility on the patch for "
                    "these arguments, and maybe the patch should be updated."
                )
...
def find_transformer_model_type(model_class: Type) -> str:
    ...
    if not module_name.startswith("paddlefleet.transformers."):
        return default_model_type
...
def find_transformer_model_class_by_name(
    ...
    """
    transformer_module = import_module("paddlefleet.transformers")
    ...
    *,
    ...
    if is_url(repo_id):
        ...
        if url_file_exists(repo_id):
            ...
            download_check(repo_id, "paddlefleet_hub_download")
    ...
    if url_file_exists(community_model_file_path):
        ...
        # # check the downloaded weight file and registered weight file name
        download_check(community_model_file_path, "paddlefleet_hub_download")
        return weight_file_path
...
def cached_file(
    ...
    if download_hub == DownloadSource.AISTUDIO:
        ...
        try:
            ...
            resolved_file = paddlefleet_hub_download(
                path_or_repo_id,
                filename,
...
def paddlefleet_load(path, map_location="cpu"):
