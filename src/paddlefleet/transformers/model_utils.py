...
def load_state_dict(
    ...
    state_dict = paddlefleet_load(checkpoint_file, map_location="cpu")
    return state_dict
...
def register_base_model(cls):
    ...
    Example:
        .. code-block::
            ...
            from paddlefleet.transformers import BertModel, register_base_model
...
def load_sharded_checkpoint(
    ...
    loader = (
        safe_load_file
        if load_safe
        else partial(paddlefleet_load, map_location="cpu")
    )
...
def _load_state_dict_into_model(
    ...
    with warnings.catch_warnings(record=True) as w:
        warnings.resetwarnings()
        # paddlefleet hold  missing_keys , just ignore not found warnings.
        warnings.filterwarnings(
            "ignore", message=r".*is not found in the provided dict.*"
        )
...
class PretrainedModel(Layer, GenerationMixin, ConversionMixin):
    ...
    def _post_init(self, original_init, *args, **kwargs):
        ...
        if (
            original_init.__module__ != "paddlefleet.transformers.model_utils"
    ...
    def set_inference_config(cls, config, predictor_args, **kwargs):
        ...
        if predictor_args.speculate_method is not None:
            ...
            if predictor_args.speculate_method is not None:
                if not config.get("speculate_model_type", "None") in [
                    "eagle",
                    "mtp",
                ]:
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *args, **kwargs):
        ...
        Example:
            .. code-block::
                ...
                from paddlefleet.transformers import BertForSequenceClassification
        ...
        if load_checkpoint_format == "flex_checkpoint":
            ...
            if (
                ...
                if flex_ckpt_comm_method_resolved is None:
                    flex_ckpt_comm_method_resolved = (
        ...
        **kwargs,
        ...
        if config_to_save is None:
            config_to_save = copy.deepcopy(
                getattr(model_to_save, "config_to_save", model_to_save.config)
            )
...
class PipelinePretrainedModel(PretrainedModel):
    ...
    def _set_pipeline_name_mapping(self, mappings=None):
        if mappings is not None:
            ...
            shared_layer_names = {
                layer.layer_name
                for layer in self._layers_desc
...
def load_sharded_checkpoint_as_one(folder, variant=None, return_numpy=False):
    ...
    loader = (
        ...
        if load_safe
            ...
            paddlefleet_load, map_location="np" if return_numpy else "cpu"
        )
    )
...
def clean_model_class_name(
    class_name, suffixes_to_strip: Union[str, List[str]] = "Pipe"
):
    """
