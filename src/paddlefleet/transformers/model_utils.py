...
def load_sharded_checkpoint(model, folder, variant=None, strict=True, prefer_safe=False):
    ...
    loader = safe_load_file if load_safe else partial(paddlefleet_load, map_location="cpu")
...
def load_sharded_checkpoint_as_one(folder, variant=None, return_numpy=False):
    ...
    shard_files = list(set(index["weight_map"].values()))
    loader = safe_load_file if load_safe else partial(paddlefleet_load, map_location="np" if return_numpy else "cpu")
