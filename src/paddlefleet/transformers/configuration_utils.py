...
class PretrainedConfig:
    ...
    - **model_type** (`str`) -- An identifier for the model type, serialized into the JSON file, and used to recreate
      the correct object in [`~paddlefleet.AutoConfig`].
    - **is_composition** (`bool`) -- Whether the config class is composed of multiple sub-configs. In this case the
      config has to be initialized from two or more configs of type [`~paddlefleet.PretrainedConfig`] like:
      [`~paddlefleet.EncoderDecoderConfig`] or [`~RagConfig`].
    ...
    Arg:
        ...
        return_dict (`bool`, *optional*, defaults to `True`):
            Whether or not the model should return a [`~paddlefleet.transformers.model_outputs.ModelOutput`] instead of a plain tuple.
        ...
        return_dict_in_generate (`bool`, *optional*, defaults to `False`):
            Whether the model should return a [`~paddlefleet.transformers.model_outputs.ModelOutput`] instead of a `paddlefleet.Tensor`.
    ...
    def __init__(self, **kwargs):
        ...
        self.paddlefleet_version = kwargs.pop("paddlefleet_version", None)
    ...
    def use_return_dict(self) -> bool:
        """
        `bool`: Whether or not return [`~paddlefleet.transformers.model_outputs.ModelOutput`] instead of tuples.
        """
        ...
        Args:
            pretrained_model_name_or_path (`str` or `os.PathLike`):
                ...
                - a string, the *model id* of a pretrained model configuration hosted inside a model repo on
                  paddlefleet bos server. Valid model ids can be located at the root-level, like `bert-base-uncased`, or
    ...
    def to_dict(self, saving_file=False) -> Dict[str, Any]:
        ...
        # PaddleFleet version when serializing the model
        output["paddlefleet_version"] = __version__

        for key, value in output.items():
            # Deal with nested configs like CLIP
            if isinstance(value, PretrainedConfig):
                value = value.to_dict()
                del value["paddlefleet_version"]
...
def get_configuration_file(configuration_files: List[str]) -> str:
    """
    Get the configuration file to use for this version of paddlefleet.
    ...
    paddlefleet_version = version.parse(__version__)
    for v in available_versions:
        if version.parse(v) <= paddlefleet_version:
...
def layer_type_validation(
    layer_types: List[str], num_hidden_layers: Optional[int] = None
):
