...
class DeepseekV4Config(PretrainedConfig):
    ...
    Args:
        ...
        indexcache_topk_pattern (`str`, *optional*):
            Producer-first F/S pattern over ratio=4 CSA layers; None disables IndexCache.
        indexcache_multi_layer_distill (`bool`, *optional*, defaults to `False`):
            Distill served-layer attention targets onto the producer Indexer.
        ...
        def migrate(values):
            ...
            def normalize(value):
                ...
                if not isinstance(value, str):
                    raise ValueError(
                        "IndexCache pattern must be a string or None."
                    )
            ...
            if legacy is not None and current is not None and legacy != current:
                raise ValueError(
                    "Checkpoint index_topk_pattern and indexcache_topk_pattern disagree."
                )
            values["indexcache_topk_pattern"] = (
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if "index_topk_pattern" in kwargs:
            raise ValueError(
                "Use indexcache_topk_pattern; load legacy checkpoints through from_dict/from_pretrained."
            )
