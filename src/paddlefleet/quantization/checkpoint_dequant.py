...
class CheckpointDequantizer(Protocol):
    ...
    def configure_formats(
        self, value_format: str, scale_format: str
    ...
    def logical_shape(
        self, physical_shape: tuple[int, ...]
    ) -> tuple[int, ...]: ...

    def physical_qweight_shape(
        self, logical_shape: tuple[int, ...]
    ) -> tuple[int, ...]: ...
...
_UE8M0_LUT = tuple(
    float("inf") if exponent == 255 else 2.0 ** (exponent - 127)
...
def _as_uint8_codes(
    tensor: paddle.Tensor, component_name: str
) -> paddle.Tensor:
...
def _decode_e2m1_packed(tensor: paddle.Tensor) -> paddle.Tensor:
    ...
    high = paddle.bitwise_and(
        paddle.bitwise_right_shift(raw, paddle.full_like(raw, 4)),
        paddle.full_like(raw, 0xF),
    )
    codes = paddle.stack([low, high], axis=-1).reshape(
...
def _require_components(
    components: dict[str, paddle.Tensor], required: set[str]
) -> None:
    missing = required - set(components)
    if missing:
        raise KeyError(
            f"Missing checkpoint quantization components: {sorted(missing)}."
        )
...
def _expand_scale_grid(
    ...
    for axis, block_size in sorted(axis_to_block.items()):
        expanded = paddle.repeat_interleave(
            expanded, repeats=block_size, axis=axis
        )
...
class _ConfiguredCheckpointDequantizer:
    ...
    def configure_formats(
        ...
        if value_format not in self.value_formats:
            raise ValueError(
                f"Unsupported {self.method} value_format {value_format!r}."
            )
        if scale_format not in self.scale_formats:
            raise ValueError(
                f"Unsupported {self.method} scale_format {scale_format!r}."
            )
        return type(self)(
            value_format, scale_format, self.block_axes, self.block_shape
        )
    ...
    def configure_geometry(
        ...
        if (
            not block_axes
            or len(block_axes) != len(block_shape)
            or len(set(block_axes)) != len(block_axes)
        ):
            raise ValueError(f"Invalid block geometry for {self.method}.")
        if any(axis < 0 for axis in block_axes) or any(
            size <= 0 for size in block_shape
        ):
            raise ValueError(f"Invalid block geometry for {self.method}.")
        return type(self)(
            self.value_format,
    ...
    def _geometry(self, rank: int) -> tuple[tuple[int, ...], tuple[int, ...]]:
        ...
        axes = tuple(
            axis + rank if axis < 0 else axis for axis in self.block_axes
        )
        if any(axis >= rank for axis in axes):
            raise ValueError(
                f"Invalid block_axes {self.block_axes} for a rank-{rank} tensor."
            )
    ...
    def _scales(
        self, scale: paddle.Tensor, logical_shape: tuple[int, ...]
    ) -> paddle.Tensor:
...
class FP8BlockCheckpointDequantizer(_ConfiguredCheckpointDequantizer):
    ...
    def dequantize(
        ...
        return (
            qweight * self._scales(components["scale"], logical_shape)
        ).astype(output_dtype)
...
class MXFP4GroupCheckpointDequantizer(_ConfiguredCheckpointDequantizer):
    ...
    def physical_qweight_shape(
        ...
        if not logical_shape or logical_shape[-1] % 2:
            raise ValueError(
                f"Invalid MXFP4 logical weight shape: {logical_shape}."
            )
    ...
    def physical_qweight_slice(
        ...
        if global_offset[-1] % 2 or local_shape[-1] % 2:
            raise ValueError(
                "MXFP4 qweight slices must have even last-axis offsets and sizes."
            )
        return (
    ...
    def logical_shard_is_aligned(
        ...
        if not super().logical_shard_is_aligned(
            global_shape, local_shape, global_offset
        ):
            return False
        if global_offset[-1] % 2 or local_shape[-1] % 2:
            return False
...
def _normalize_method(method: str) -> str:
    if not isinstance(method, str):
        raise TypeError(
            f"Checkpoint quantization method must be a string, got {type(method).__name__}."
        )
...
def register_checkpoint_dequantizer(
    method: str, dequantizer: CheckpointDequantizer
) -> None:
    method = _normalize_method(method)
    if method in _CHECKPOINT_DEQUANTIZERS:
        raise ValueError(
            f"Checkpoint dequantizer {method!r} is already registered."
        )
    if not callable(getattr(dequantizer, "dequantize", None)):
        raise TypeError(
            "Checkpoint dequantizer must provide a callable dequantize() method."
        )
...
register_checkpoint_dequantizer(
    "mxfp4_group", MXFP4GroupCheckpointDequantizer()
)
