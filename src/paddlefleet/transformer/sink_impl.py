...
def _merge_kv_grad(grad_repeated, original, num_key_value_groups):
    if (
        ...
        and grad_repeated.shape[2]
        == original.shape[2] * num_key_value_groups
    ):
...
class FlashMaskSinkPyLayer(PyLayer):
    ...
    def forward(
        ...
        *,
        ...
        final_output = raw_output * multiplier.transpose([0, 2, 1]).unsqueeze(-1)
    ...
    def backward(ctx, grad_output):
        ...
        grad_v = _merge_kv_grad(
            grad_v_repeated, value, num_key_value_groups
        )
