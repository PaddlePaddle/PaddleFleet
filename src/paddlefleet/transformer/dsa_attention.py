    ...
    def forward(
        ...
        if (
            ...
            if self.config.use_accuracy_compatible:
                # Dummy, not k_latent: live PIR CSE'd key[..., :v] to
                # k_latent. The accuracy-compatible replay re-slices V out
                # of the absorbed key itself, so the operand handed in here
                # only carries the shape.
                value = paddle.zeros(k_latent.shape, dtype=k_latent.dtype)
            else:
                # Without that replay the attention keeps whatever value it
                # is handed, so a dummy would zero the output. key_abs is
                # concat([k_latent, k_rope]), so k_latent is exactly the
                # leading slice the replay would have taken.
                value = k_latent
