    ...
    def get_query_key_value_tensors(
        ...
        if kv_combined.size(-1) != self.kv_lora_rank + self.qk_rope_head_dim:
            ...
            if (
                ...
                # kv_compressed:[b, s / TP, kv_lora_rank]. Under
                # use_accuracy_compatible the constructor clears
                # kv_b_proj.sequence_parallel instead, so the sequence stays
                # gathered here; every other path keeps this scatter so the
                # KV branch matches the q branch above.
        ...
        def qkv_up_proj_and_rope_apply(
            ...
            if getattr(self.config, "mla_use_nope", False):
                ...
                # The segment goes in as hooks rather than
                # `q[..., n:] = inspect_tensor(..., q[..., n:])`: that spelling
                # slices and writes into `q` in place even with the probes off,
                # which bumps its inplace version and breaks backward /
                ...
                q = inspect_tensor(
                    ...
                    post_load_func=lambda seg, full=q: scatter_last_dim_segment(
