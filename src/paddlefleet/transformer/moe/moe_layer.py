...
class MoELayer(nn.Layer):
    def __init__(
        ...
        if config.moe_token_dispatcher_type == "allgather":
            # AllGather dispatcher always uses the SonicMoE fused expert path.
            if not paddlefleet_ops.is_sonic_moe_available():
                raise ValueError(
                    paddlefleet_ops.blocked_import_messages.get(
                        "paddlefleet_ops.sonicmoe",
        ...
        if self.expert_model_parallel_size > 1:
            if self.moe_token_dispatcher_type in ("deepep", "hybridep"):
                ...
                # 'allgather' mode: every expert is sharded along its
                # intermediate dim across the EP group; AllGather inputs and
                # ReduceScatter outputs around the (grouped) expert compute.
                # Only supported with EP>1 and sonic_moe; fp8 is handled by run_sonic_moe internally.
                ...
                if (
                    ...
                    raise ValueError(
                        ...
                        f"{self.expert_model_parallel_size} in 'allgather' "
                        f"mode."
                    )
                # 'allgather' path now supports moe_shared_expert_overlap by
                # fusing the EP ReduceScatter (combine_postprocess) with the
                # shared-expert subgraph via ``_AllGatherCombineAsync``. The
                # AllGather dispatcher uses the ``fusion_moe_forward`` control
                # flow (same as deepep), with SonicMoEExpert handling the
                # expert compute via ``run_sonic_moe``.
            else:
                logger.info(
        ...
        if use_fused_weight:
            if self.moe_token_dispatcher_type == "allgather":
                # In allgather mode every rank holds a shard of *every*
                # expert along the intermediate dim, so the fused expert
                # must allocate weights for all num_experts (not just
                # num_local_experts) and use the per-EP-rank shard size.
    ...
    def _init_expert_parallel(self):
        ...
        if self.expert_model_parallel_size > 1:
            ...
            if self.moe_token_dispatcher_type == "allgather":
                # In 'allgather' mode every expert is sharded along its
                # intermediate dim across the EP group; every rank therefore
                # holds (a shard of) *every* expert.
    ...
    def custom_forward(
        ...
        # When the AllGather overlap path is active the projection was already
        # hoisted into forward() (before pre_allgather), so we skip it here.
        if (
            self.use_latent_moe
            and getattr(self, "_latent_hidden", None) is None
        ):
            hidden_states = self.fc1_latent_proj(hidden_states)
        elif self.use_latent_moe and self._latent_hidden is not None:
        ...
        if should_log_balance and global_moe_balance_training_logs_enabled():
            if self.moe_token_dispatcher_type == "allgather":
                # AllGather dispatcher has no `_comm_manager`; instead every
                # rank holds the full `_global_topk_indices` after the
                # AllGather. Re-derive a per-rank chunk so log_moe_balance's
                # internal all_gather reconstructs the same flat
                # `[num_experts]` global view that deepep/alltoall produce
                # (their `tokens_per_expert` is `[num_local_experts]` and is
                # all_gather'd into `[ep_size, num_local_experts]`).
                global_topk_indices = self.token_dispatcher._global_topk_indices
                global_counts = paddle.bincount(
                    global_topk_indices.reshape([-1]).cast("int64"),
                    minlength=self.num_experts,
                )
                ep_size = self.expert_model_parallel_size
                tokens_per_expert = global_counts.reshape([ep_size, -1])[
                    self.moe_rank
                ]
            else:
                tokens_per_expert = (
    ...
    def fusion_moe_forward(
        ...
        if self.moe_token_dispatcher_type == "allgather":
            # AllGather dispatcher: get indices/probs from the global
            # AllGather results rather than from a comm_manager.
            dispatched_indices = self.token_dispatcher._global_topk_indices
            dispatched_probs = self.token_dispatcher._global_topk_weights
            if (
                should_log_balance
                and global_moe_balance_training_logs_enabled()
            ):
                global_counts = paddle.bincount(
                    dispatched_indices.reshape([-1]).cast("int64"),
                    minlength=self.num_experts,
                )
                ep_size = self.expert_model_parallel_size
                tokens_per_expert = global_counts.reshape([ep_size, -1])[
        ...
        with profile("combine"):
            if self.moe_token_dispatcher_type == "allgather":
                ...
                hidden_states = self.token_dispatcher.combine_postprocess(
                    hidden_states
                )
            else:
    ...
    def dispatch_preprocess(self, args):
        ...
        if not isinstance(self.token_dispatcher, MoEFlexTokenDispatcher):
            raise TypeError(
                f"token_dispatcher must be MoEFlexTokenDispatcher, got {type(self.token_dispatcher)}"
            )
    ...
    def forward(
        ...
        # Overlap hidden_states AllGather with gate computation:
        # issue the async AllGather on the comm stream now; gate runs on
        # the calc stream while the AllGather is in flight.  The result is
        # consumed inside dispatch_preprocess via _PreAllGatherResult.
        # For latent MoE: fc1_latent_proj is hoisted here so the AllGather
        # targets the latent-space tensor; gate still runs on the original
        # hidden_states.
        if (
