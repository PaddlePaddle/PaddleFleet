...
class MoELayer(nn.Layer):
    def __init__(
        ...
        # === MG对齐 (PF_MOE_ALIGN_MG=1) ===
        # MG baseline 走 alltoall + MOE_DETERMINISTIC_UNPERMUTE=1 (gather+sum fp32)。
        # PF 默认 deepep dispatcher 后 routing_map 形状/topk 可变，无法直接套 MG 的
        # deterministic 算法。这里强制 PF 也走 alltoall，使 forward 进 custom_forward →
        # AllToAllTokenDispatcher，combine_postprocess 调 moe_utils.unpermute 的
        # PF_MOE_ALIGN_MG 分支 (gather+sum) → 与 MG 1:1 对齐。
        if _is_bit_exact():
            if self.moe_token_dispatcher_type != "alltoall":
                logger.info(
                    "[PF_MOE_ALIGN_MG=1] forcing moe_token_dispatcher_type "
                    "%s -> alltoall for byte-exact alignment with Megatron baseline.",
    ...
    def expert_forward(
        ...
        if use_accuracy_compatible_kernel():
            ...
            if per_token_scale is None:
                # DeepEP path (e.g. MoEFlexTokenDispatcher): probs are applied
                # inside unpermute() (gather+sum). For PF_MOE_ALIGN_MG=1 we
                # force the alltoall dispatcher upstream, so this branch is
                # only hit by deepep paths; alltoall always sets
                # global_input_probs and matches MG's per-expert probs path.
                pass
            else:
        ...
        for i, chunk in enumerate(chunks):
            ...
            # === 小 M 专家 FFN 跨框架 byte-exact 对齐 (受 GLM_ALIGN_BIT_EXACT 控制) ===
            # paddle cublasLt 在 fc1 ([M,4096]@[4096,2816] bf16) 的 M=16 处会选一个
            # 专用 tiny-M kernel, 其 tiling/累加序与 torch(:16:8) 分叉
            # (PF=adfc.. vs MG=0b2c..); 而 M>=17 两框架 byte-exact。torch 对 M 不敏感
            # (恒走大 kernel), 故只需 PF 端把 M<17 的输入零填充到 _pad_to(=32) 行,
            # 让 paddle 也走大 kernel, 算完切回前 M 行。零填充行 fc1(0)→swiglu→fc2 仍只
            # 影响自身(GEMM 行间唯一耦合就是 kernel 选择), 已离线验证 pad{17..64} 切片后
            # == MG 原生 M=16 (0b2c..)。MG 端无需改 (pad-invariant)。
            _pad_to = 32 if _is_bit_exact() else 0
            _m_real = int(chunk.shape[0])
            _do_pad = _pad_to > 0 and 0 < _m_real < 17 and _m_real < _pad_to
            _scale_i = None if scale_chunks is None else scale_chunks[i]
            if _do_pad:
                _padrows = _pad_to - _m_real
                chunk = paddle.concat(
                    [
                        chunk,
                        paddle.zeros(
                            [_padrows, chunk.shape[1]], dtype=chunk.dtype
                        ),
                    ],
                    axis=0,
                )
                if _scale_i is not None:
                    _scale_i = paddle.concat(
                        [
                            _scale_i,
                            paddle.zeros([_padrows], dtype=_scale_i.dtype),
                        ],
                        axis=0,
                    )
            if _scale_i is None:
                ...
                expert_output = expert(chunk, per_token_scale=_scale_i)[0]
            if _do_pad:
                expert_output = expert_output[:_m_real]
            # === 诊断插桩: per-expert FFN 输入/输出 md5 (global expert idx) ===
            # 定位 rank4 expert_raw_output 分叉具体落在哪个 expert。
            # 用 global idx 作为跨 rank/跨框架 join key。受 GLM_ALIGN_LOG 控制。
            if _is_log_enabled():
                _ln_pe = getattr(self, "layer_number", None)
                # 用真实(未 pad)的行做 md5, 名称 token 数也用真实值, 与 MG 可比。
                _chunk_real = chunk[:_m_real] if _do_pad else chunk
                _nt_pe = _m_real
                _pf_ti(
                    f"perexp_e{current_expert_idx}_nt{_nt_pe}_in",
                    _chunk_real,
                    _ln_pe,
                    prefix="PF MoE",
                )
                _pf_ti(
                    f"perexp_e{current_expert_idx}_nt{_nt_pe}_out",
                    expert_output,
                    _ln_pe,
                    prefix="PF MoE",
                )
    ...
    def routed_experts_compute(
        ...
        # === 诊断插桩: 对齐 MG moe_layer.py:256 permuted_input ===
        _pf_ti("permuted_input", global_input_tokens, _ln, prefix="PF MoE")
        if not global_input_tokens.stop_gradient:
            global_input_tokens.register_hook(
                _pf_grad_info(
                    "ep_permuted_input_to_experts",
                    layer_num=_ln,
                    prefix="GRAD PF MoE",
                )
            )
        ...
        # === 诊断插桩: 对齐 MG moe_layer.py:263 expert_raw_output ===
        _pf_ti("expert_raw_output", expert_outs, _ln, prefix="PF MoE")
        if not expert_outs.stop_gradient:
            expert_outs.register_hook(
                _pf_grad_info(
                    "ep_expert_raw_output", layer_num=_ln, prefix="GRAD PF MoE"
                )
            )
        result = self.unpermute(expert_outs)
        if not result.stop_gradient:
            result.register_hook(
                _pf_grad_info(
                    "ep_after_combine_preprocess",
                    layer_num=_ln,
                    prefix="GRAD PF MoE",
                )
            )
    ...
    def custom_forward(
        ...
        if not hidden_states.stop_gradient:
            hidden_states.register_hook(
                _pf_grad_info(
                    "ep_after_dispatch", layer_num=_ln, prefix="GRAD PF MoE"
                )
            )
        if not hidden_states.stop_gradient:
            hidden_states.register_hook(
    ...
    def fusion_moe_forward(
        ...
        # === MG对齐 (PF_MOE_ALIGN_MG=1): 在 fusion_moe_forward 入口直接重定向到
        # custom_forward，用 gather+sum(dim=1) (moe_utils.unpermute 对齐分支) 输出。
        # 原 fusion 路径走 FusionMoePyLayer → fused moe_unpermute C++ op，与 MG baseline
        # (gather+sum) 在 bf16 累加序上不一致，导致 routed_output ~2e-2 diff。
        # custom_forward 走 AllToAllTokenDispatcher.combine_postprocess → unpermute()
        # 的 PF_MOE_ALIGN_MG 分支 (1:1 复刻 MG gather+sum)。
        if _is_bit_exact():
