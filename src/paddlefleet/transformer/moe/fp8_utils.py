...
def split_group_gemm(
    ...
    Args:
        ...
        use_ue8m0 (bool): Whether to use UE8M0 scale format (requires Blackwell GPU).

    Returns:
        paddle.Tensor, shape=(N, T): 返回计算结果存储在gemm_out中的张量。
    """
    print("[sd] debug split_group_gemm use_ue8m0 ", use_ue8m0)
    assert use_ue8m0
    start_idx = 0
    for i, token_num in enumerate(tokens_per_expert):
        ...
        if use_ue8m0:
            # ue8m0: weight scale needs TMA alignment (double transpose for contiguity)
            w_scale_tma_align = w_scale[i].T.contiguous().T
            deep_gemm.gemm_fp8_fp8_bf16_nt(
                (x_fp8[start_idx:end_idx], x_scale_tma_align),
                (w_fp8[i], w_scale_tma_align),
...
class ExpertsGroupGemmContiguousNode:
    ...
    def __init__(
        ...
        Args:
            ...
            use_ue8m0 (bool, optional): Whether to use UE8M0 scale format. Requires Blackwell GPU. Defaults to False.
        """
    ...
    def fwd_gate_up_fp8(
        ...
        if numpy.prod(x_fp8.shape) != 0:
            if not self.moe_grouped_gemm:
                ...
                if self.use_ue8m0:
                    # ue8m0: grouped gemm scale needs extra transpose for TMA alignment
                    w1_t_scale = (
    ...
    def bwd_down_input_fp8(
        ...
        if self.use_ue8m0 and self.moe_grouped_gemm:
            # grouped-gemm (deep_gemm) path on Blackwell with ue8m0 requires
            # the scale's penultimate-dim stride to be 1 (sf.stride(-2) == 1).
            # Mirror ernie-core's reference impl: quantize with
            # output_scale_transpose=True, then transpose back via .T so the
            # resulting scale has the layout DeepGEMM expects.
    ...
    def bwd_gate_up_input_fp8(self, do1, expert_w1, dx=None):
        ...
        # quant do1
        if self.use_ue8m0 and self.moe_grouped_gemm:
            # grouped-gemm path on Blackwell with ue8m0: DeepGEMM requires
            # sf.stride(-2) == 1. Use output_scale_transpose=True + .T to
            # produce the correct layout (mirrors ernie-core reference).
            do1_fp8, do1_scale = (
                paddle.incubate.nn.functional.fp8_quant_blockwise(
                    do1,
                    output_scale_transpose=True,
    ...
    def fused_transpose_split_quant(
        ...
        if self.use_ue8m0:
            ...
            def _tma_aligned_size(n, element_size):
                return _align(n, 16 // element_size)

            out_list, scale_list = [], []
            for tokens in tokens_per_expert:
                out_list.append(
                    paddle.empty([x.shape[1], tokens], dtype="float8_e4m3fn")
                )
                aligned_mn = _tma_aligned_size(
                    x.shape[1], 4
                )  # int32 element_size=4
                aligned_k = _ceil_div(tokens, 128 * 4)
                scale_list.append(
                    paddle.empty([aligned_k, aligned_mn], dtype="int32")
                )
            fuse_transpose_split_fp8_quant(
                x,
                None,
    ...
    def bwd_gate_up_weight(self, do1, input_x, expert_w1, clear_input=False):
        ...
        if input_x is None:
            if self.dequant_input:
                if self.use_ue8m0:
                    # ue8m0: int32 scale cannot be passed to fused_transpose_split_quant;
                    # dequantize back to bf16 first, then re-quantize
                    input_x = paddle.incubate.nn.functional.fused_act_dequant(
