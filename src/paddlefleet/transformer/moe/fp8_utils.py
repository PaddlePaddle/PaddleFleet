...
class ExpertsGroupGemmContiguousNode:
    ...
    def __init__(
        ...
        if self.backward_subbatch_rows is not None:
            assert (
                self.backward_subbatch_rows > 0 and self.backward_subbatch_rows % FP8_ALIGN == 0
    ...
    def bwd_down_input_fp8(self, expert_w2, unzipped_grad, o1, unzipped_probs, inplace_swiglu_prob=False):
        """
        do2 = do3 * w2_t
    ...
    def bwd_gate_up_weight(self, do1, input_x, expert_w1, clear_input=False):
        ...
        if input_x is None:
            if self.dequant_input:
                ...
                input_x_t_fp8, input_x_t_scale = self.fused_transpose_split_quant(self.input, None, self.tokens_per_expert, True)
        else:
    ...
    def forward(self, hs_out, unzipped_probs, tokens_per_expert, origin_token_per_experts, output=None, scale=None):
        ...
        # o3
        o3 = self.fwd_down(o1, unzipped_probs, expert_w2, num_expert, clear_o1=clear_o1)
        return o3
    ...
    def backward(self, out_grad, unzipped_probs, a2a_async_fn=None):
        ...
        assert a2a_async_fn is None, "a2a_async_fn should be None when backward_subbatch_rows is not None"
