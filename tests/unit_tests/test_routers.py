...
class TestTop2Router:
    ...
    def test_router_forward(self, moe_router_pre_softmax, score_function):
        with paddle.no_grad():
            ...
            hidden_states = paddle.randn(
                (32, 2, self.router.config.hidden_size)
            )
    ...
    @pytest.mark.internal
    @pytest.mark.skipif(
        not paddle.cuda.is_available() or not HAVE_ROUTER_FUSION,
    ...
    def test_router_forward_fusion_equivalence(self, score_function):
        with paddle.no_grad():
            ...
            assert paddle.equal(routing_ref, routing_fused), (
                "Routing map mismatch"
            )
            assert paddle.allclose(scores_ref, scores_fused)
            # restore the config
            self.router.config.moe_router_fusion = False
    ...
    def test_aux_loss(self):
        ...
        # Without aux loss
        hidden_states = paddle.randn((32, 2, self.router.config.hidden_size))
        hidden_states = hidden_states.cuda().bfloat16()
    ...
    def test_router_dtype(self):
        ...
        self.sequential_mlp = self.sequential_mlp.cuda()
        hidden_states = paddle.randn(
            (32, 2, self.router.config.hidden_size), dtype=paddle.bfloat16
        )
        ...
        with paddle.no_grad():
            ...
            assert scores.dtype == paddle.bfloat16, (
                "Router output should be bf16 by default"
            )
            assert out[0].dtype == paddle.bfloat16
        ...
        with paddle.no_grad():
            scores, routing_map = self.router(hidden_states)
            out = self.sequential_mlp(hidden_states)
            assert scores.dtype == paddle.float32, (
        ...
        with paddle.no_grad():
            scores, routing_map = self.router(hidden_states)
            out = self.sequential_mlp(hidden_states)
            assert scores.dtype == paddle.float64, (
    ...
    def test_force_load_balancing(self):
        ...
        assert not paddle.allclose(normal_scores, force_scores)
    ...
    def test_token_dropping(
        self, capacity_factor, drop_policy, pad_to_capacity
    ):
        if capacity_factor is None and pad_to_capacity:
            pytest.skip(
                "Capacity factor is None, so no token dropping should be applied"
            )
        ...
        hidden_states = paddle.randn(
            (num_tokens, self.router.config.hidden_size),
            dtype=paddle.bfloat16,
            device="cuda",
        )
        hidden_states.requires_grad = True
        ...
        if capacity_factor is not None:
            if pad_to_capacity:
                assert (
                    ...
                    == num_tokens
                    * self.router.config.moe_router_topk
                    * capacity_factor
                )
            else:
                assert (
            ...
            assert (
                routing_map.sum().item()
                == num_tokens * self.router.config.moe_router_topk
            )
...
class TestGroupLimitedRouter:
    ...
    @pytest.mark.parametrize(
        "moe_router_group_topk,moe_router_num_groups", [(3, 8), (2, 4)]
    )
    @pytest.mark.parametrize("moe_router_pre_softmax", [(True), (False)])
    @pytest.mark.parametrize("score_function", ["sigmoid", "softmax"])
    def test_router_forward(
        ...
        with paddle.no_grad():
            ...
            group_routing_map = (
                routing_map.reshape(num_tokens, moe_router_num_groups, -1)
                .max(dim=-1)
                .values
            )
            assert paddle.all(
                group_routing_map.sum(dim=-1) <= moe_router_group_topk
            )
...
class TestAuxLossFreeTop2Router:
    ...
    def test_router_forward_aux_free(self):
        ...
        assert not paddle.equal(initial_bias, updated_bias), (
    ...
    def test_router_forward_fusion_equivalence(self, score_function):
        with paddle.no_grad():
            ...
            expert_bias_sample = paddle.randn_like(router_ref.expert_bias)
            ...
            hidden_states = paddle.randn((32, 2, router_ref.config.hidden_size))
            hidden_states = hidden_states.cuda().bfloat16()
...
@pytest.mark.parametrize(
    "router_dtype", [paddle.bfloat16, paddle.float32, paddle.float64]
)
def test_router_gating_linear(router_dtype):
    tols = dict(rtol=2.0e-2, atol=1.0e-3)

    ref_inp = paddle.randn((4096, 7168), dtype=paddle.bfloat16, device="cuda")
    ref_weight = paddle.randn((256, 7168), dtype=paddle.bfloat16, device="cuda")
    ref_inp.requires_grad = True
    ref_weight.requires_grad = True
    bwd_input = paddle.randn((4096, 256), dtype=router_dtype, device="cuda")

    ref_output = paddle.nn.functional.linear(
        ref_inp.to(router_dtype), ref_weight.to(router_dtype)
    )
    ...
    assert paddle.allclose(output, ref_output, **tols)
    assert paddle.allclose(inp.grad, ref_inp.grad, **tols)
    assert paddle.allclose(weight.grad, ref_weight.grad, **tols)
...
def test_router_gating_linear_bias(router_dtype):
    ...
    ref_inp = paddle.randn((4096, 7168), dtype=router_dtype, device="cuda")
    ref_weight = paddle.randn((256, 7168), dtype=router_dtype, device="cuda")
    ref_bias = paddle.randn((256,), dtype=router_dtype, device="cuda")
    ref_inp.requires_grad = True
    ref_weight.requires_grad = True
    ...
    ref_output = paddle.nn.functional.linear(
        ref_inp.to(router_dtype),
        ref_weight.to(router_dtype),
        ref_bias.to(router_dtype),
    )
    ref_output.backward(bwd_input)
    ...
    assert paddle.allclose(inp.grad, ref_inp.grad, **tols)
    assert paddle.allclose(weight.grad, ref_weight.grad, **tols)
    assert paddle.allclose(bias.grad, ref_bias.grad, **tols)
