...
if [[ ${step} == "pt" ]]; then
  ...
  config_yaml=$root_dir/PaddleFleet/tests/config/ci/glm45_pt.yaml
  export data_dir=$root_dir/PaddleFleet/tests/fixtures/dummy/pt
  ...
  config_yaml=$root_dir/PaddleFleet/tests/config/ci/glm45_sft.yaml
  export data_dir=$root_dir/PaddleFleet/tests/fixtures/dummy/sft
  ...
  config_yaml=$root_dir/PaddleFleet/tests/config/ci/glm45_lora.yaml
  export data_dir=$root_dir/PaddleFleet/tests/fixtures/dummy/sft
  ...
  config_yaml=$root_dir/PaddleFleet/tests/config/ci/glm45_dpo.yaml
  export data_dir=$root_dir/PaddleFleet/tests/fixtures/dummy/dpo
  yq '.train_dataset_path = strenv(data_dir) + "/train.jsonl"
      | .eval_dataset_path = strenv(data_dir) + "/eval.jsonl"
      | .model_name_or_path = strenv(cur_dir) + "/checkpoints/glm_full_pp_ckpts"
  ...
  export data_dir=$root_dir/PaddleFleet/tests/fixtures/dummy/dpo
  config_yaml=$root_dir/PaddleFleet/tests/config/ci/glm45_dpo_lora.yaml
...
if [ $exit_code -ne 0 ]; then
    ...
    python $root_dir/PaddleFleet/tests/check_log_for_exitcode.py ./${log_file} "***** train metrics *****"
    check_log_exit_code=$?
...
python $root_dir/PaddleFleet/tests/integration_test/check_loss.py \
   --compare_step 10 \
   --log_file ./${log_file} \
...
if [ $? -ne 0 ]; then
  ...
  source /root/proxy
  bash $root_dir/PaddleFleet/tests/integration_test/check_precision_approval.sh
  if [ $? -ne 0 ]; then
