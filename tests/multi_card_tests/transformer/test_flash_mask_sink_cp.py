...
class TestRefinedRecomputeFlashMaskSinkCP(unittest.TestCase):
    """rr-CP FlashMask attention + learnable_sink, driven through recompute.

    The rr-CP path needs two forward passes (first stashes tensors under
    no_grad, second rebuilds the graph during recompute's backward). Wrapping
    the rr-CP call in ``recompute`` forces both. We compare against the non-rr
    ...
    def _run(self, batch_size, seq_len, nheads, d, sink_trainable=True):
        ...
        # rr-CP path driven through recompute (forces both forward passes).
        q_local = _scatter_leaf(q_full)
        k_local = _scatter_leaf(k_full)
        v_local = _scatter_leaf(v_full)
        sink_local = sink_full.detach().clone()
        ...
        out_local = recompute(_fwd, q_local, k_local, v_local, use_reentrant=True)
        ...
        if sink_trainable:
            ...
            # Fixed sink is stop_gradient -> rr-CP returns 3-tuple, no grad.
            self.assertIsNone(sink_local.grad)
