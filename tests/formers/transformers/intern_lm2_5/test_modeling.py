...
from tests.formers.testing_utils import require_package, slow
...
class InternLM25ModelTest(unittest.TestCase):
    ...
    def test_model_save_and_load(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            loaded_model = InternLM25ForCausalLM.from_pretrained(
...
class InternLM25ConvertedTest(unittest.TestCase):
    ...
    def test_hf_direct_load_and_inference(self):
        ...
        decoded = tokenizer.decode(
            generated_ids[0].numpy().tolist(), skip_special_tokens=True
        )
...
class InternLM25CompatibilityTest(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        config = AutoConfig.from_pretrained(
            modelscope_lm25_model_location, trust_remote_code=True
        )
...
class InternLM2ProxyEmbeddingTest(unittest.TestCase):
    ...
    def test_model_get_input_embeddings(self):
        ...
        self.assertEqual(
            emb.weight.shape, [self.config.vocab_size, self.config.hidden_size]
        )
    ...
    def test_for_causal_lm_get_output_embeddings(self):
        ...
        self.assertEqual(
            out.weight.shape, [self.config.hidden_size, self.config.vocab_size]
        )

    def test_for_causal_lm_set_output_embeddings(self):
        model = InternLM2ForCausalLM(self.config)
        new_head = paddle.nn.Linear(
            self.config.hidden_size, 3000, bias_attr=False
        )
    ...
    def test_for_sequence_classification_get_input_embeddings(self):
        ...
        self.assertIsNotNone(emb)
        self.assertEqual(
            emb.weight.shape, [config.vocab_size, config.hidden_size]
        )
