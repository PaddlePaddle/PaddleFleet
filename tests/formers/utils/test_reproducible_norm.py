...
def test_layout_and_chunk_invariance():
    norm = ReproducibleL2Norm()
    gradient = (
        paddle.arange(1, 12289, dtype="float32").reshape([96, 128]) / 16384
    )
    whole = norm.accumulate(norm.zeros(), gradient)
    transposed = norm.accumulate(
        norm.zeros(), gradient.transpose([1, 0]), chunk_size=123
    )
...
def test_hybrid_owner_filter_and_coefficient(nested):
    ...
    gradients = [
        ...
        paddle.to_tensor([12.0]),
    ]
    params = [
        _parameter("first"),
        _parameter("second"),
        _parameter("shared", is_firstly_shared=False),
    ]
    wrapper._dygraph_clip(list(zip(params, gradients)))
    assert wrapper.stat["global_grad_norm"] == 5.0
    expected = paddle.to_tensor([3.0, 4.0, 12.0]) * (
        1.0 / (paddle.to_tensor([5.0]) + 1e-6)
    )
    assert paddle.equal_all(paddle.concat(gradients), expected).item()


@pytest.mark.parametrize(
    "accuracy,limit,expected",
...
def test_trainer_clip_selection(accuracy, limit, expected):
    ...
    trainer.model = SimpleNamespace(
        config=SimpleNamespace(use_accuracy_compatible=accuracy)
    )
...
def test_trainer_norm_logging_keeps_the_clipping_recipe(monkeypatch, moe):
    ...
    if moe:
        ...
        clip = HybridParallelClipGrad(
            HybridParallelClipGrad(inner_clip, hcg=None), hcg=None
        )
    ...
    monkeypatch.setattr(
        ReproducibleHybridParallelClipGrad, "_global_norm", lambda *args: None
    )
    if moe:
    ...
    trainer.args = SimpleNamespace(
        use_expert_parallel=False, max_grad_norm=1.0, train_mtp_only=False
    )
    ...
    expected = paddle.to_tensor([3.0, 4.0]) * (
        1.0 / (paddle.to_tensor([5.0]) + 1e-6)
    )
