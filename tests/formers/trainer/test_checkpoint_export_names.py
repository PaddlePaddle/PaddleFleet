...
class _FakeArgs:
    def __init__(
        self,
        output_dir,
        save_hf_output_dir=None,
        save_hf_total_limit=None,
...
class ResolveHfCheckpointDirTests(unittest.TestCase):
    def test_default_is_nested_under_output_dir(self):
        ...
        self.assertTrue(
            path.startswith(os.path.abspath(output_dir) + os.sep)
            or path.startswith(output_dir + os.sep)
        )
    ...
    def test_opt_in_override_leaves_output_dir(self):
        ...
        path = resolve_hf_checkpoint_dir(
            output_dir, 5, save_hf_output_dir=other
        )
...
class UniqueNameTests(unittest.TestCase):
    ...
    def test_duplicate_nested_names_fail_for_oracle_root(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_tiny_safetensors(
                os.path.join(
                    tmp, "hf_checkpoint-5", "model-00001-of-00001.safetensors"
                ),
                payload,
            )
...
class CadenceLayoutAndRotationTests(unittest.TestCase):
    ...
    def test_trainer_opt_in_cadence_root(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            trainer = _bare_trainer(
                os.path.join(tmp, "checkpoint"), save_hf_output_dir=other
            )
    ...
    def test_rotate_hf_checkpoints_deletes_oldest_under_output_dir(self):
        with tempfile.TemporaryDirectory() as tmp:
            for step in (1, 2, 3):
                os.makedirs(
                    os.path.join(tmp, f"hf_checkpoint-{step}"), exist_ok=True
                )
            trainer = _bare_trainer(tmp, save_hf_total_limit=2)
            trainer._rotate_hf_checkpoints(output_dir=tmp)
            remaining = sorted(
                p for p in os.listdir(tmp) if p.startswith("hf_checkpoint-")
            )
