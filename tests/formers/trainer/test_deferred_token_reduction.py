...
class TestDeferredTokenReduction(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        missing = [
            ...
            if not callable(getattr(language_loss, name, None))
        ]
        if missing:
            raise unittest.SkipTest(
                "Requires PaddleFleet deferred token normalization support; missing: "
                + ", ".join(missing)
            )
    ...
    def parameter(self, name, main=True):
        ...
        return SimpleNamespace(
            name=name,
            main_grad=grad if main else None,
            grad=None if main else grad,
        )
    ...
    def bucket(self, degree, use_avg=True):
        return SimpleNamespace(
            _comm_group=SimpleNamespace(nranks=degree), _use_reduce_avg=use_avg
        )
    ...
    def test_unsharded_main_and_fallback_grad_keep_token_division(self):
        parameters = [
            self.parameter("main"),
            self.parameter("fallback", main=False),
        ]
        self.apply(SimpleNamespace(), parameters)
        self.assertEqual(
            [self.values(p) for p in parameters], [[0.5, 1.0], [0.5, 1.0]]
        )
    ...
    def test_per_parameter_avg_and_sum_scale_groups(self):
        for use_avg in (True, False):
            with self.subTest(use_avg=use_avg):
                dense, expert = (
                    self.parameter("dense"),
                    self.parameter("expert"),
                )
                opt = self.sharded(
                    {
                        "dense": [self.bucket(2, use_avg)],
                        "expert": [self.bucket(1, use_avg)],
                    }
                )
    ...
    def test_all_mappings_checked_before_first_gradient_changes(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "missing FusedCommBuffer"):
            self.apply(
                self.sharded({"first": [self.bucket(2)]}), [first, missing]
            )
