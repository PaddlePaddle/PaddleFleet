...
from paddlefleet.trainer.trainer_utils import (
...
class TestAllGatherOnDevice(unittest.TestCase):
    ...
    def setUpClass(cls):
        if (
            ...
            raise unittest.SkipTest(
                "all_gather_on_device is a device-only path"
            )
    ...
    def setUp(self):
        ...
        self._old_bucket = (
            reshard_common._STATE_DICT_BROADCAST_BUCKET_SIZE_BYTES
        )
    ...
    def _sources(self):
        return {
            name: _rand_bf16(shape, seed)
            for seed, (name, shape) in enumerate(SHAPES.items())
        }

    def _sink(self):
        return {
            name: paddle.full(shape, -7.0, dtype=paddle.bfloat16)
    ...
    def _host_reference(self):
        ...
        out = all_gather_state_dict(
            {k: v.cpu() for k, v in self._sources().items()},
            lambda x: True,
            self.group,
        )
    ...
    def test_matches_the_host_path_bitwise(self):
        ...
        sink = self._sink()
        out = all_gather_on_device(
            dict(self._sources()), self.group, sink, max_chunk_bytes=self.chunk
        )
        ...
        for name, shape in SHAPES.items():
            self.assertIsInstance(
                out[name],
                AssignedMasterWeight,
                f"{name} did not land in its parameter",
            )
            self.assertEqual(out[name].shape, shape)
            np.testing.assert_array_equal(
                _f32(sink[name]), reference[name], err_msg=name
            )
    ...
    def test_no_parameter_is_left_unwritten(self):
        ...
        for name, param in sink.items():
            self.assertFalse(
                bool(paddle.any(paddle.cast(param, paddle.float32) == -7.0)),
                name,
            )
    ...
    def test_values_survive_the_bucket_being_freed(self):
        ...
        for name, param in sink.items():
            np.testing.assert_array_equal(
                _f32(param), snapshot[name], err_msg=name
            )
    ...
    def test_everything_returned_is_on_device(self):
        ...
        self.assertFalse(
            out["p_b"].place.is_cpu_place(),
            "the fallback tensor must stay on device",
        )

    def test_source_consumed(self):
        sources = dict(self._sources())
        all_gather_on_device(
            sources, self.group, self._sink(), max_chunk_bytes=self.chunk
        )
        self.assertEqual(
            len(sources), 0, "sources must be released as they are packed"
        )
    ...
    def test_shape_mismatch_does_not_touch_the_parameter(self):
        reference = self._host_reference()
        sink = self._sink()
        sink["p_vec"] = paddle.full(
            [32], -7.0, dtype=paddle.bfloat16
        )  # wrong length
        out = all_gather_on_device(
            dict(self._sources()), self.group, sink, max_chunk_bytes=self.chunk
        )
        ...
        np.testing.assert_array_equal(_f32(out["p_vec"]), reference["p_vec"])
        self.assertTrue(
            bool(paddle.all(paddle.cast(sink["p_vec"], paddle.float32) == -7.0))
        )
    ...
    def test_empty_sink_returns_plain_device_tensors(self):
        reference = self._host_reference()
        out = all_gather_on_device(
            dict(self._sources()), self.group, None, max_chunk_bytes=self.chunk
        )
        for name, value in out.items():
            self.assertNotIsInstance(value, AssignedMasterWeight)
            self.assertFalse(value.place.is_cpu_place())
            np.testing.assert_array_equal(
                _f32(value), reference[name], err_msg=name
            )
    ...
    def test_bigger_buckets_mean_fewer_broadcasts(self):
        ...
        few, _ = reshard_common._build_state_dict_broadcast_buckets(
            meta, 1024 * 1024
        )
        ...
        self.assertEqual(
            len(few),
            1,
            "one owner and one dtype should collapse into one bucket",
        )
...
class TestDeviceGatherSwitch(unittest.TestCase):
    ...
    def test_off_makes_the_2d_restore_decline(self):
        set_device_gather(False)
        self.assertIsNone(
            _restore_master_weights_2d_on_device({}, _SingleRankGroup(), {})
        )
    ...
    def test_does_not_go_through_all_gather_state_dict(self):
        ...
        body = src.replace(
            doc, ""
        )  # the docstring mentions the host entry point by name
