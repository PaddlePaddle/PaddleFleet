...
SPEC = importlib.util.spec_from_file_location(
    "checkpoint_export", ROOT / "src/paddlefleet/trainer/checkpoint_export.py"
)
...
class ExportProvenanceTests(unittest.TestCase):
    def test_live_provider_and_aoa_snapshot_are_serializable_and_independent(
        self,
    ):
        ...
        record = EXPORT.hf_export_provenance(
            config, aoa, "results/checkpoint", 2
        )
        json.dumps(record)
        ...
        self.assertEqual(
            record["output_dir"], os.path.abspath("results/checkpoint")
        )
        config.indexer_types.append("changed")
        aoa["aoa_statements"].clear()
        self.assertEqual(
            record["provider_config"]["indexer_types"], ["full", "shared"]
        )
        self.assertEqual(
            record["aoa_config"]["aoa_statements"],
            ["internal.weight^T -> official.weight"],
        )

    def test_missing_is_distinct_from_false_or_none_and_not_completion(self):
        record = EXPORT.hf_export_provenance(
            SimpleNamespace(moe_expert_fusion=False, params_dtype=None),
            {},
            "checkpoint",
        )
