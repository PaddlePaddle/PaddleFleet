...
if [[ ${step} == "pt" ]]; then
  ...
  config_yaml=$root_dir/PaddleFleet/tests/formers/config/ci/glm45_pt.yaml
  export data_dir=$root_dir/PaddleFleet/tests/formers/fixtures/dummy/pt
  ...
  config_yaml=$root_dir/PaddleFleet/tests/formers/config/ci/glm45_sft.yaml
  export data_dir=$root_dir/PaddleFleet/tests/formers/fixtures/dummy/sft
  ...
  config_yaml=$root_dir/PaddleFleet/tests/formers/config/ci/glm45_lora.yaml
  export data_dir=$root_dir/PaddleFleet/tests/formers/fixtures/dummy/sft
  ...
  config_yaml=$root_dir/PaddleFleet/tests/formers/config/ci/glm45_dpo.yaml
  export data_dir=$root_dir/PaddleFleet/tests/formers/fixtures/dummy/dpo
  yq '.train_dataset_path = strenv(data_dir) + "/train.jsonl"
      | .eval_dataset_path = strenv(data_dir) + "/eval.jsonl"
      | .model_name_or_path = strenv(cur_dir) + "/checkpoints/glm_full_pp_ckpts"
  ...
  export data_dir=$root_dir/PaddleFleet/tests/formers/fixtures/dummy/dpo
  config_yaml=$root_dir/PaddleFleet/tests/formers/config/ci/glm45_dpo_lora.yaml
...
set +e
FLAGS_use_stride_compute_kernel=False NNODES=1 MASTER_ADDR=$master MASTER_PORT=$port CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 coverage run $(which paddlefleet-cli) train $config_yaml 2>&1 | tee ./${log_file}
...
if [ $exit_code -ne 0 ]; then
    ...
    python $root_dir/PaddleFleet/tests/formers/check_log_for_exitcode.py ./${log_file} "***** train metrics *****"
    check_log_exit_code=$?
...
python $root_dir/PaddleFleet/tests/formers/integration_test/check_loss.py \
   --compare_step 10 \
   --log_file ./${log_file} \
...
if [ $? -ne 0 ]; then
  ...
  source /root/proxy
  bash $root_dir/PaddleFleet/tests/formers/integration_test/check_precision_approval.sh
  if [ $? -ne 0 ]; then
