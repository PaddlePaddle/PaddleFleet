...
config_dpo_yaml=$root_dir/PaddleFleet/tests/formers/config/ci/glm45_dpo_lora.yaml
...
set +e
NNODES=1 MASTER_ADDR=$master MASTER_PORT=$port coverage run $(which paddlefleet-cli) train $config_dpo_yaml 2>&1 | tee ./${log_file}
sft_exit_code=$?
if [ $sft_exit_code -ne 0 ]; then
   echo "GLM4.5 multi-cards training failed, try to check the log file"
   python $root_dir/PaddleFleet/tests/formers/check_log_for_exitcode.py ./${log_file} "***** eval metrics *****"
   sft_check_exit_code=$?
