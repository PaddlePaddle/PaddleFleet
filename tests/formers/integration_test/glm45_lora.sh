...
config_lora_yaml=$root_dir/PaddleFleet/tests/formers/config/ci/glm45_lora.yaml
...
set +e
NNODES=1 MASTER_ADDR=$master MASTER_PORT=$port coverage run $(which paddlefleet-cli) train $config_lora_yaml 2>&1 | tee ./${log_file}
