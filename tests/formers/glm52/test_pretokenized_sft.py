...
def test_model_repro_observation_callback_projects_paddle_57_44(
    ...
    monkeypatch.setattr(
        "paddlefleet.cli.train.sft.workflow.paddle.distributed.get_rank",
        lambda: 0,
    )
    ...
    receipt = __import__("json").loads(
        (tmp_path / "input_receipt.json").read_text()
    )
    assert receipt["semantic"]["input_token_count"] == 57
    assert receipt["semantic"]["supervised_target_count"] == 44
    assert (
        receipt["semantic"]["projection"]
        == "dataset_row_before_paddle_padding_and_label_roll"
    )
...
def test_layer0_fine_forward_specs_are_explicit_and_fail_closed():
    assert (
        ModelReproObservationCallback._forward_contract_specs("coarse") is None
    )
    ...
    with pytest.raises(
        ValueError, match="unsupported MODEL_REPRO_FORWARD_BOUNDARY_SET"
    ):
...
def test_first_tensor_prefers_first_tensor_in_nested_module_output():
    ...
    assert (
        ModelReproObservationCallback._first_tensor(
            (None, {"output": first}, second)
        )
        is first
    )
