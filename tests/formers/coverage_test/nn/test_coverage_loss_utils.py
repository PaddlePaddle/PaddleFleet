...
class TestSubbatch(unittest.TestCase):
    """Tests for paddlefleet.nn.criterion.loss_utils.subbatch."""

    def test_subbatch_small_input_no_split(self):
        """When input is smaller than batch size, should call function directly."""
        from paddlefleet.nn.criterion.loss_utils import subbatch
...
class TestCalcLMHeadLogits(unittest.TestCase):
    """Tests for paddlefleet.nn.criterion.loss_utils.calc_lm_head_logits."""
    ...
    def test_calc_lm_head_logits_basic(self):
        ...
        from paddlefleet.nn.criterion.loss_utils import calc_lm_head_logits

        config = self._make_config()
        hidden = paddle.randn([2, 8, 64], dtype="float32")
        weight = paddle.randn([100, 64], dtype="float32")
        with patch(
            "paddlefleet.nn.criterion.loss_utils.parallel_matmul"
        ) as mock_matmul:
    ...
    def test_calc_lm_head_logits_sequence_parallel(self):
        ...
        weight = paddle.randn([100, 64], dtype="float32")
        with patch(
            "paddlefleet.nn.criterion.loss_utils.GatherOp"
        ) as mock_gather:
