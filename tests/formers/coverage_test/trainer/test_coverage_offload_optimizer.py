...
import paddlefleet.trainer.utils.offload_optimizer as offload_optimizer_module
...
class TestOffload(unittest.TestCase):
    ...
    def test_offload_cpu(self):
        ...
        with patch(
            "paddlefleet.trainer.utils.offload_optimizer.paddle.is_compiled_with_cuda",
            return_value=False,
        ):
            with patch(
                "paddlefleet.trainer.utils.offload_optimizer.paddle.is_compiled_with_xpu",
                return_value=False,
            ):
                offload(x)
...
class TestMuonMasterWeightLifecycle(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        self._restore_optional(
            Muon, "_muon_update_group", self._snapshot["group"]
        )
    ...
    def _run_group_wrapper(
        self, stub, group_params_grads, hack_fn=hack_offload_optimizer
    ):
        ...
        ``hack_fn`` selects the standard or eb5 patch entry point."""
        events = []
        Muon._muon_update_group = lambda self, gpg, *a, **k: events.append(
            ("update",)
        )
        hack_fn()
    ...
    def _run_update_wrapper(
        self, stub, param, momentum_buffer, hack_fn=hack_offload_optimizer
    ):
    ...
    def test_update_group_reloads_master_before_update_and_offloads_when_enabled(
        self,
    ):
    ...
    def test_update_group_offloads_only_enabled_params_in_mixed_group(self):
        ...
        events = self._run_group_wrapper(
    ...
    def test_muon_update_reloads_master_before_update_and_offloads_when_enabled(
        self,
    ):
    ...
    def test_eb5_update_group_reloads_master_before_update_and_offloads_when_enabled(
        self,
    ):
        """eb5 _muon_update_group reloads master weight before the update and offloads it after when is_offload_opt=True."""
        mw, mom = paddle.zeros([2, 2]), paddle.zeros([2, 2])
        param = _StubParam("w0", is_offload_opt=True)
        stub = _StubMuon({"w0": mw}, {"w0": mom})
        ...
        events = self._run_group_wrapper(
            ...
            hack_fn=hack_offload_optimizer_eb5,
        )
    ...
    def test_eb5_update_group_skips_master_offload_when_disabled(self):
        """eb5 _muon_update_group still reloads master weight but never offloads it when is_offload_opt=False."""
        mw, mom = paddle.zeros([2, 2]), paddle.zeros([2, 2])
        param = _StubParam("w0", is_offload_opt=False)
        stub = _StubMuon({"w0": mw}, {"w0": mom})
    ...
    def test_eb5_muon_update_reloads_master_before_update_and_offloads_when_enabled(
        self,
    ):
        """The eb5 _muon_update fallback reloads master weight before the update and offloads it after when is_offload_opt=True."""
        mw, mom = paddle.zeros([2, 2]), paddle.zeros([2, 2])
        param = _StubParam("w0", is_offload_opt=True)
        stub = _StubMuon({"w0": mw}, {"w0": mom})

        events = self._run_update_wrapper(
            stub, param, mom, hack_fn=hack_offload_optimizer_eb5
        )
    ...
    def test_eb5_muon_update_skips_master_offload_when_disabled(self):
        """The eb5 _muon_update fallback still reloads master weight but never offloads it when is_offload_opt=False."""
        mw, mom = paddle.zeros([2, 2]), paddle.zeros([2, 2])
        param = _StubParam("w0", is_offload_opt=False)
        stub = _StubMuon({"w0": mw}, {"w0": mom})
