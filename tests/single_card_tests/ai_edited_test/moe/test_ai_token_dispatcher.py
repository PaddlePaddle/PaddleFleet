...
class TestTokenDispatcher(unittest.TestCase):
    ...
    def test_deep_ep_backend_defaults_to_deepep_and_ignores_env(self):
        """Test backend selection is controlled by config, not env vars."""
        from paddlefleet.transformer.moe.token_dispatcher import (
        ...
        with (
            ...
            patch(
                "paddlefleet.transformer.moe.token_dispatcher.HAVE_HYBRID_EP",
                True,
            ),
        ):
            self.assertEqual(get_selected_deep_ep_backend_name(), "deepep")

    def test_deep_ep_backend_selects_hybrid_explicitly(self):
        """Test explicit config selects HybridEP backend."""
        from paddlefleet.transformer.moe.token_dispatcher import (
            get_selected_deep_ep_backend_name,
    ...
    def test_deep_ep_backend_deep_ep_v2_is_reserved(self):
        """Test reserved deep_ep_v2 backend is explicit but unsupported."""
        from paddlefleet.transformer.moe.token_dispatcher import (
            get_selected_deep_ep_backend_name,
        )
    ...
    def test_moe_flex_token_dispatcher_uses_hybrid_backend(self):
        """Test MoEFlexTokenDispatcher chooses HybridEP from config."""
        from paddlefleet.transformer.moe.token_dispatcher import (
            MoEFlexTokenDispatcher,
        )
        ...
        with (
            ...
            patch(
                "paddlefleet.transformer.moe.token_dispatcher._HybridEPManager",
                mock_manager_cls,
            ),
        ):
            dispatcher = MoEFlexTokenDispatcher(
            ...
            self.assertIs(
                dispatcher._comm_manager, mock_manager_cls.return_value
            )
            self.assertEqual(
                mock_manager_cls.call_args.kwargs["needs_host_counts"], True
            )

    def test_hybrid_ep_manager_combine_rejects_overlap(self):
        """Test HybridEP combine rejects fake overlap handles."""
        from paddlefleet.transformer.moe.token_dispatcher import (
            _HybridEPManager,
        )
    ...
    def test_hybrid_ep_manager_combine_clears_runtime_state(self):
        """Test HybridEP combine calls the sync bridge and clears state."""
        from paddlefleet.transformer.moe.token_dispatcher import (
            _HybridEPManager,
        )
        ...
        combined = object()
        with patch(
            "paddlefleet.transformer.moe.token_dispatcher.hybrid_ep_combine",
