...
class TestTransformerBlockConstruction(unittest.TestCase):
    ...
    @patch(
        "paddlefleet.transformer.transformer_block.ProcessGroupCollection.use_mpu_process_groups"
    )
    @patch("paddlefleet.transformer.transformer_block.build_spec_layer")
...
class TestTransformerBlockMethods(unittest.TestCase):
    ...
    def _make_block(self):
        with (
            ...
            patch(
                "paddlefleet.transformer.transformer_block.build_spec_layer"
            ) as mock_build,
        ):
            mock_pg.return_value = MagicMock()
