...
def apply_mla_rope_for_q_ref(
    ...
    q: [seq, bs, heads, qk_head_dim + emb_dim] (sbhd)
    ...
    if cu_seqlens_q is None:
        # sbhd: gather cos/sin along seq axis
        seq_len = q.shape[0]
        cos_row = cos_f32[:seq_len].reshape([seq_len, 1, 1, emb_dim])
        sin_row = sin_f32[:seq_len].reshape([seq_len, 1, 1, emb_dim])
...
def apply_mla_rope_for_kv_ref(
    ...
    kv:        [seq, bs, heads, k_dim + v_dim] or [T, heads, k_dim + v_dim]
    k_pos_emb: [seq, bs, 1, emb_dim]           or [T, 1, emb_dim]
    cos/sin:   [max_seq, 1, 1, emb_dim]
    ...
    if cu_seqlens_kv is None:
        ...
        heads = kv.shape[2]
        rope_emb = rope_emb.expand([seq_len, kv.shape[1], heads, emb_dim])
    else:
