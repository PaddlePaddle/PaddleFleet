...
from paddlefleet.generation.greedy_generator import GreedyGenerator, _sample_next_token
...
def _fleet_init():
    ...
    strategy.hybrid_configs = {
        ...
        "sharding_degree": 1, "sep_degree": 1, "cp_degree": 1,
        "ep_degree": 1, "moe_sharding_degree": 1,
...
def _make_model(vocab_size: int = 64, hidden_size: int = 64,
                num_layers: int = 2, max_seq_len: int = 32):
...
class TestSampleNextTokenGreedy(unittest.TestCase):
    ...
    def test_temperature_zero_returns_argmax(self):
        tok = _sample_next_token(self._logits(3), temperature=0.0, top_k=0, top_p=0.0)
        self.assertEqual(tok.shape, [1, 1])
        self.assertEqual(int(tok[0, 0]), 3)

    def test_negative_temperature_returns_argmax(self):
        tok = _sample_next_token(self._logits(7), temperature=-1.0, top_k=0, top_p=0.0)
        self.assertEqual(int(tok[0, 0]), 7)

    def test_top_k_one_returns_argmax(self):
        tok = _sample_next_token(self._logits(5), temperature=1.0, top_k=1, top_p=0.0)
...
class TestSampleNextTokenTopK(unittest.TestCase):
    def test_top_k_constrains_output(self):
        ...
        allowed = {0, 1, 2}
        for _ in range(50):
            tok = _sample_next_token(logits, temperature=1.0, top_k=3, top_p=0.0)
            self.assertIn(int(tok[0, 0]), allowed)

    def test_top_k_equal_vocab_no_filtering(self):
        vocab = 8
        logits = paddle.randn([1, vocab])
        tok = _sample_next_token(logits, temperature=1.0, top_k=vocab, top_p=0.0)
...
class TestSampleNextTokenTopP(unittest.TestCase):
    def test_top_p_near_zero_picks_top_token(self):
        ...
        logits[0, 4] = 10.0
        for _ in range(20):
            tok = _sample_next_token(logits, temperature=1.0, top_k=0, top_p=0.01)
...
class TestSampleNextTokenTemperature(unittest.TestCase):
    def _unique_count(self, temperature: float, n_runs: int = 200) -> int:
        ...
        for _ in range(n_runs):
            tok = _sample_next_token(logits, temperature=temperature, top_k=0, top_p=0.0)
            counts.add(int(tok[0, 0]))
        return len(counts)

    def test_high_temperature_more_diverse(self):
        self.assertGreaterEqual(self._unique_count(5.0), self._unique_count(0.1))
...
class TestSampleNextTokenEdgeCases(unittest.TestCase):
    def test_single_token_vocab(self):
        tok = _sample_next_token(paddle.zeros([1, 1]), temperature=1.0, top_k=0, top_p=0.0)
        self.assertEqual(int(tok[0, 0]), 0)

    def test_output_within_vocab(self):
        vocab = 50
        tok = _sample_next_token(paddle.randn([2, vocab]), temperature=0.8, top_k=10, top_p=0.0)
        for i in range(2):
            self.assertTrue(0 <= int(tok[i, 0]) < vocab)

    def test_top_k_and_top_p_combined(self):
        tok = _sample_next_token(paddle.randn([1, 30]), temperature=1.0, top_k=10, top_p=0.9)
...
class TestGreedyGeneratorGenerate(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.eos_id = 2
        cls.input_ids = paddle.to_tensor([[1, 5, 10, 3]], dtype="int64")  # [1, 4]
    ...
    def test_generated_tokens_in_vocab(self):
        ...
        for t in gen_tokens:
            self.assertTrue(0 <= t < self.vocab_size, f"token {t} out of vocab range")
    ...
    def test_max_new_tokens_not_exceeded(self):
        ...
        out = self.gen.generate(
            self.input_ids, max_new_tokens=max_new,
    ...
    def test_eos_stops_generation(self):
        ...
        out = self.gen.generate(
            self.input_ids, max_new_tokens=32, eos_token_id=self.eos_id,
            temperature=0.0,
        )
...
class TestSampleNextTokenGreedy(unittest.TestCase):
    ...
    def test_temperature_zero_returns_argmax(self):
        logits = self._logits(peak_idx=3)
        tok = _sample_next_token(logits, temperature=0.0, top_k=0, top_p=0.0)
        self.assertEqual(tok.shape, [1, 1])
        self.assertEqual(int(tok[0, 0]), 3)

    def test_negative_temperature_returns_argmax(self):
        logits = self._logits(peak_idx=7)
        tok = _sample_next_token(logits, temperature=-1.0, top_k=0, top_p=0.0)
        self.assertEqual(int(tok[0, 0]), 7)

    def test_top_k_one_returns_argmax(self):
        logits = self._logits(peak_idx=5)
        tok = _sample_next_token(logits, temperature=1.0, top_k=1, top_p=0.0)
        self.assertEqual(int(tok[0, 0]), 5)

    def test_greedy_batch(self):
        """Batch of 3, each has a different peak — all must be recovered greedily."""
        logits = paddle.zeros([3, 10])
        peaks = [2, 6, 9]
        for i, p in enumerate(peaks):
            logits[i, p] = 10.0
...
class TestSampleNextTokenTopK(unittest.TestCase):
    """top_k filtering: sampled token must be within the top-k set."""

    def test_top_k_constrains_output(self):
        vocab = 20
        top_k = 3
        # Tokens 0,1,2 get high logits; rest get very negative logits.
        logits = paddle.full([1, vocab], -100.0)
        logits[0, 0] = 3.0
        logits[0, 1] = 2.0
        logits[0, 2] = 1.0
        ...
        rng = paddle.seed(42)
        for _ in range(50):
            tok = _sample_next_token(logits, temperature=1.0, top_k=top_k, top_p=0.0)
            self.assertIn(int(tok[0, 0]), allowed,
                          f"sampled token {int(tok[0,0])} outside top-{top_k} set")

    def test_top_k_equal_vocab_no_filtering(self):
        """top_k == vocab_size should not raise and should return valid token."""
        vocab = 8
        logits = paddle.randn([1, vocab])
        tok = _sample_next_token(logits, temperature=1.0, top_k=vocab, top_p=0.0)
        self.assertTrue(0 <= int(tok[0, 0]) < vocab)
...
class TestSampleNextTokenTopP(unittest.TestCase):
    ...
    def test_top_p_near_zero_picks_top_token(self):
        """top_p=0.01 → only the single highest-prob token survives the nucleus."""
        vocab = 20
        logits = paddle.full([1, vocab], -100.0)
        logits[0, 4] = 10.0  # overwhelming probability mass on token 4
    ...
    def test_top_p_one_allows_all_tokens(self):
        """top_p=1.0 is effectively disabled; function should not raise."""
        logits = paddle.randn([1, 10])
        tok = _sample_next_token(logits, temperature=1.0, top_k=0, top_p=1.0)
        self.assertTrue(0 <= int(tok[0, 0]) < 10)

    def test_top_p_disabled_when_zero(self):
        """top_p=0.0 → nucleus filter is skipped entirely."""
        vocab = 10
        logits = paddle.randn([1, vocab])
        tok = _sample_next_token(logits, temperature=1.0, top_k=0, top_p=0.0)
        self.assertTrue(0 <= int(tok[0, 0]) < vocab)
...
class TestSampleNextTokenTemperature(unittest.TestCase):
    """Higher temperature → flatter distribution → more diverse outputs."""

    def _entropy_over_runs(self, temperature: float, n_runs: int = 200) -> float:
        vocab = 10
        logits = paddle.zeros([1, vocab])
        logits[0, 0] = 2.0  # slight bias toward token 0
        counts = [0] * vocab
        for _ in range(n_runs):
            tok = _sample_next_token(logits, temperature=temperature, top_k=0, top_p=0.0)
            counts[int(tok[0, 0])] += 1
        unique = sum(1 for c in counts if c > 0)
        return unique

    def test_high_temperature_more_diverse(self):
        unique_low = self._entropy_over_runs(temperature=0.1)
        unique_high = self._entropy_over_runs(temperature=5.0)
        self.assertGreaterEqual(unique_high, unique_low,
                                "high temperature should produce >= unique tokens than low temperature")
...
class TestSampleNextTokenEdgeCases(unittest.TestCase):
    ...
    def test_single_token_vocab(self):
        logits = paddle.zeros([1, 1])
        tok = _sample_next_token(logits, temperature=1.0, top_k=0, top_p=0.0)
        self.assertEqual(int(tok[0, 0]), 0)

    def test_output_within_vocab(self):
        vocab = 50
        logits = paddle.randn([2, vocab])
        tok = _sample_next_token(logits, temperature=0.8, top_k=10, top_p=0.0)
        for i in range(2):
            self.assertTrue(0 <= int(tok[i, 0]) < vocab)

    def test_top_k_and_top_p_combined(self):
        """top_k and top_p applied sequentially should not crash and stay in vocab."""
        vocab = 30
        logits = paddle.randn([1, vocab])
        tok = _sample_next_token(logits, temperature=1.0, top_k=10, top_p=0.9)
        self.assertTrue(0 <= int(tok[0, 0]) < vocab)
