...
Tolerance policy:
  - float32: atol = rtol = 1e-9.  Both paths compute entirely in fp32 and
    should agree to near machine-epsilon.
  - float16/bfloat16: atol = rtol = 1e-9 when comparing in the NATIVE dtype.
    Comparison is done by converting both tensors back to float32 first; at
    that point the bit-patterns of the fp16/bf16 outputs are promoted losslessly
    to float32, so the delta should again be 0 when both implementations follow
    identical rounding paths.
    NOTE: The Python reference uses Paddle's high-level `tile`/`unsqueeze` ops
    which internally call into cuBLAS-level kernels for cosf/sinf; our CUDA
    kernel calls CUDA device `cosf/sinf` directly.  These two paths can produce
    results that differ by ±1 ULP in float32, leading to a ±1 ULP difference
    in the final cast to fp16/bf16 (worst-case ~0.4% relative error for bf16).
    We therefore use atol = rtol = 1e-9 only for float32, and allow a small
    but tight tolerance for fp16/bf16 equal to 2× the machine epsilon of the
    respective dtype:
      float16  eps ≈ 9.77e-4  → atol = rtol = 2e-3
      bfloat16 eps ≈ 7.81e-3  → atol = rtol = 2e-2
...
# ---------------------------------------------------------------------------
# Tests for custom_ops.fused_apply_rotary_pos_emb_vision
# ---------------------------------------------------------------------------


class TestFusedRopeVisionForward(unittest.TestCase):
    """Forward tests for fused_apply_rotary_pos_emb_vision from custom_ops."""
    ...
    def _run_fwd(self, batch, seq, heads, dim, dtype):
        ...
        self.assertEqual(list(cuda_out.shape), list(ref_out.shape))
        atol, rtol = _TOL[dtype]
        np.testing.assert_allclose(
            cuda_out.numpy().astype("float32"),
            ref_out.numpy().astype("float32"),
            rtol=rtol,
            atol=atol,
            err_msg=f"fused fwd mismatch: {batch}x{seq}x{heads}x{dim} {dtype}",
        )

    def test_fwd_float32(self):
        self._run_fwd(4, 256, 8, 64, "float32")

    def test_fwd_float16(self):
        self._run_fwd(4, 256, 8, 64, "float16")

    def test_fwd_bfloat16(self):
        self._run_fwd(4, 256, 8, 64, "bfloat16")

    def test_fwd_large(self):
        self._run_fwd(8, 512, 16, 128, "bfloat16")


class TestFusedRopeVision3D(unittest.TestCase):
    """3D input tests for fused_apply_rotary_pos_emb_vision (actual usage pattern)."""
    ...
    def _run_fwd_3d(self, seq, heads, dim, dtype, freqs_dtype="float32"):
        ...
        freqs_f32 = freqs.astype("float32")
        ref_out = apply_rotary_pos_emb_vision_ref(tensor_4d, freqs_f32).squeeze(
            0
        )
        ...
        np.testing.assert_allclose(
            ...
            rtol=rtol,
            atol=atol,
            err_msg=f"fused 3D fwd mismatch: {seq}x{heads}x{dim} {dtype} freqs={freqs_dtype}",
        )

    def test_3d_bf16_tensor_bf16_freqs(self):
        """Matches real usage: q=bf16, rotary_pos_emb=bf16."""
        self._run_fwd_3d(256, 8, 64, "bfloat16", "bfloat16")

    def test_3d_bf16_tensor_fp32_freqs(self):
        self._run_fwd_3d(256, 8, 64, "bfloat16", "float32")

    def test_3d_float32(self):
        self._run_fwd_3d(256, 8, 64, "float32")

    def test_3d_float16(self):
        self._run_fwd_3d(256, 8, 64, "float16")

    def test_3d_large(self):
        self._run_fwd_3d(1024, 16, 128, "bfloat16")


class TestFusedRopeVisionBackward(unittest.TestCase):
    """Backward tests for fused_apply_rotary_pos_emb_vision from custom_ops."""
    ...
    def _run_bwd(self, batch, seq, heads, dim, dtype):
        ...
        freqs_ref = paddle.to_tensor(freqs_np, place="gpu")
        out_ref = apply_rotary_pos_emb_vision_ref(tensor_ref, freqs_ref)
        d_out_ref = paddle.to_tensor(d_out_np, place="gpu").astype(dtype)
        grad_ref = paddle.grad(
            outputs=[out_ref], inputs=[tensor_ref], grad_outputs=[d_out_ref]
        )[0]
        ...
        np.testing.assert_allclose(
            ...
            rtol=rtol,
            atol=atol,
            err_msg=f"fused bwd mismatch: {batch}x{seq}x{heads}x{dim} {dtype}",
        )

    def test_bwd_float32(self):
        self._run_bwd(4, 256, 8, 64, "float32")

    def test_bwd_bfloat16(self):
        self._run_bwd(4, 256, 8, 64, "bfloat16")

    def test_bwd_float16(self):
        self._run_bwd(4, 256, 8, 64, "float16")
    ...
    def _run_bwd_3d(self, seq, heads, dim, dtype, freqs_dtype="float32"):
        ...
        np.testing.assert_allclose(
            ...
            rtol=rtol,
            atol=atol,
            err_msg=f"fused 3D bwd mismatch: {seq}x{heads}x{dim} {dtype} freqs={freqs_dtype}",
        )

    def test_bwd_3d_bf16_bf16_freqs(self):
        """Matches real usage: q=bf16, rotary_pos_emb=bf16."""
        self._run_bwd_3d(256, 8, 64, "bfloat16", "bfloat16")
