...
class TestDeepGEMMFP8(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        torch.manual_seed(0)
        random.seed(0)
    ...
    def test_gemm(self):
        for (
            ...
            recipe = (
                (1, 1, 128) if (kernel_type.is_1d1d() and accumulate) else None
            )
            ...
            for test_alias in (False, True):
                ...
                func_name = (
                    f"fp8_gemm_{major_opt.lower() if test_alias else 'nt'}"
                )
                ...
                self.assertLess(
                    diff,
                    0.001,
                    msg=f"{m=}, {n=}, {k=}, {kernel_opt}, {major_opt=}, {accumulate=}, {out_dtype=}, {diff:.5f}, alias={test_alias}",
                )
        ...
        for (
            ...
            for test_alias in (False, True):
                ...
                self.assertLess(
                    diff,
                    0.001,
                    msg=f"{m=}, {n=}, {k=}, {major_opt}, {kernel_opt}, {diff:.5f}, alias={test_alias}",
                )
            ...
            for i in range(10):
                ...
                for j in range(num_groups):
                    ...
                    self.assertLess(
                        diff,
                        0.001,
                        msg=f"{max_m=}, {n=}, {k=}, {j=}, masked_m={masked_m[j]}, {kernel_opt}, {num_groups=}, {diff:.5f}",
                    )
    ...
    def test_k_grouped_gemm_contiguous(self):
        ...
        for (
            ...
            for test_empty_groups in (False, True):
                ...
                diff = calc_diff(d, ref_d)
                self.assertLess(
                    diff, 0.001, msg=f"{m=}, {n=}, {k=}, {ks=}, {diff:.5f}"
                )
