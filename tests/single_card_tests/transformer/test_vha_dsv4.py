...
``DSv4HybridAttention``, plus the config-validation ValueErrors, the
muon_slice_specs branching, and forward / backward (gradient) coverage of the
postmix and premix parameters.
"""
...
class TestPremixInit(unittest.TestCase):
    ...
    def test_disabled_builds_up_proj(self):
        attn = _build(
        ...
        self.assertTrue(attn.linear_q_up_proj.save_original_input)
...
class TestPostmixBackward(unittest.TestCase):
    """Gradients must flow to the postmix U/V params through _apply_vha_postmix.

    V is randomised (non-zero) so both U and V carry a non-trivial gradient; at
    the V=0 init the correction vanishes and dL/dU would be zero.
    """
    ...
    def _check(self, attn):
        ...
        self.assertGreater(float(attn.vha_postmix_U.grad.abs().sum()), 0.0)
        self.assertGreater(float(attn.vha_postmix_V.grad.abs().sum()), 0.0)
...
class TestPremixBackward(unittest.TestCase):
    """Gradients must flow to vha_premix_weight through the structured einsum."""
    ...
    def test_premix_weight_grad_via_einsum(self):
        ...
        g_q = attn.vha_premix_groups
        d_q = attn.vha_premix_dq
        nh = attn.num_attention_heads
        vd = attn.v_head_dim
        b, sq = 2, 5
        q_comp = paddle.randn([b, sq, attn.q_lora_rank])
        q_comp.stop_gradient = False
        # Mirrors the get_query_key_value_tensors premix branch.
        qg = q_comp.reshape([b, sq, g_q, d_q])
        q_struct = paddle.einsum(
            "btgr,krd->btkgd", qg, attn.vha_premix_weight
        ).reshape([b, sq, nh, vd])
        q_struct.sum().backward()
        self.assertIsNotNone(attn.vha_premix_weight.grad)
        self.assertGreater(float(attn.vha_premix_weight.grad.abs().sum()), 0.0)
...
class TestForward(unittest.TestCase):
    ...
    def test_backward_premix_and_postmix_params(self):
        # Full forward + backward: gradients must reach both the premix weight
        # and the postmix U/V params (V randomised so the correction is active).
        attn = _build(
            _make_config(
                use_vha_attention=True,
                use_vha_premix=True,
    ...
    def test_backward_postmix_recompute_branch(self):
        # recompute_vha_postmix True + training + backward: gradients must reach
        # the postmix params and the input through the recomputed postmix.
        attn = _build(
            _make_config(
                use_vha_attention=True,
                vha_postmix_rank=4,
