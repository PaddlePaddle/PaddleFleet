...
class TestFusedDSAIndexerLossDetachedInputs(unittest.TestCase):
    ...
    def _make_detached_inputs(self):
        ...
        for name, t in [("q", q), ("weights", weights), ("k", k),
                        ("query", query), ("key", key)]:
    ...
    def test_backward_with_all_detached_inputs_does_not_violate_pylayer_contract(self):
        ...
        try:
            loss.backward()
        except Exception as e:  # noqa: BLE001 — defend against any error type
            self.fail(
        ...
        self.assertIsNone(
            weights.grad, "weights.grad must be None for stop_gradient=True input"
        )
