...
class TestFusedTwoViewGate(unittest.TestCase):
    ...
    def _run_split(self, x_np, w0_np, w1_np):
        ...
        l0 = FusedGateDetachMatmul.apply(x, w0, False)
        l1 = FusedGateDetachMatmul.apply(x, w1, False)
        (l0.sum() + l1.sum()).backward()
        return l0, l1, x.grad, w0.grad, w1.grad

    def _run_fused(self, x_np, w0_np, w1_np):
        ...
        w1 = paddle.to_tensor(w1_np)
        w1.stop_gradient = False
        l0, l1 = FusedTwoViewGate.apply(x, w0, w1, False)
        (l0.sum() + l1.sum()).backward()
        return l0, l1, x.grad, w0.grad, w1.grad

    def _assert_bitwise(self, x_np, w0_np, w1_np):
        s = self._run_split(x_np, w0_np, w1_np)
        f = self._run_fused(x_np, w0_np, w1_np)
        names = ["logits_0", "logits_1", "x.grad", "w0.grad", "w1.grad"]
        for name, a, b in zip(names, s, f):
            self.assertEqual(
                _md5(a),
                _md5(b),
                f"{name} differs between split and fused",
            )

    def test_2d_fp32(self):
        x_np, w0_np, w1_np = self._inputs([self.N, self.D], "float32")
        self._assert_bitwise(x_np, w0_np, w1_np)
    ...
    def test_rank3_input_reshapes_back(self):
        ...
        _, _, xg, _, _ = self._run_fused(x_np, w0_np, w1_np)
    ...
    def test_bf16_input(self):
        ...
        l0, l1 = FusedTwoViewGate.apply(xb, w0, w1, False)
        (l0.sum() + l1.sum()).backward()

        s0 = FusedGateDetachMatmul.apply(xb2, w0b, False)
        s1 = FusedGateDetachMatmul.apply(xb2, w1b, False)
...
class TestFusedTwoViewGateDeferDW(unittest.TestCase):
    ...
    def test_defer_dw_matches_split_and_keeps_rank3_shape(self):
        ...
        s0 = FusedGateDetachMatmul.apply(xr, w0r, True)
        s1 = FusedGateDetachMatmul.apply(xr, w1r, True)
        (s0.sum() + s1.sum()).backward()
        ref_xg = _md5(xr.grad)
        ...
        l0d, l1d = FusedTwoViewGate.apply(xd, w0d, w1d, True)
