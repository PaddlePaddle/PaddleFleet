...
The deterministic codepath is enabled by default; this env var can be set to "0"
to opt out and fall back to the high-throughput atomic_add path.

Usage:
    python -m pytest test_csa_indexer_bwd_deterministic.py -v

Standalone:
    python test_csa_indexer_bwd_deterministic.py --repeat 20
"""
import argparse
import hashlib
import os
...
def _env_enabled():
    return os.getenv(_ENV_VAR, "1").lower() in {"1", "true", "yes", "on"}


pytestmark = pytest.mark.skipif(
    not _env_enabled(),
    reason=f"{_ENV_VAR} must not be explicitly disabled",
)
...
if __name__ == "__main__":
    ...
    if not _env_enabled():
        raise SystemExit(f"Deterministic codepath explicitly disabled via {_ENV_VAR}={os.getenv(_ENV_VAR)}")
    if paddle.is_compiled_with_cuda():
