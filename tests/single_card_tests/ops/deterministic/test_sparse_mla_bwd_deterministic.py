...
def _env_enabled():
    return all(os.getenv(name, "0").lower() in {"1", "true", "yes", "on"} for name in _REQUIRED_ENV)


pytestmark = pytest.mark.skipif(
    not _env_enabled(),
    reason="requires DSV4_TILELANG_SPARSE_MLA_DETERMINISTIC_ATTN_SINK=1 and DSV4_TILELANG_SPARSE_MLA_DETERMINISTIC_DKV=1",
)
...
@pytest.mark.skipif(
    os.getenv("DSV4_TILELANG_SPARSE_MLA_DKV_REDUCTION", "tilelang_bucketed").lower() != "tilelang_bucketed",
    reason="requires DSV4_TILELANG_SPARSE_MLA_DKV_REDUCTION=tilelang_bucketed",
)
def test_repro_tilelang_bucketed_dkv_normal():
    run_repro(mode="normal", repeat=3)
...
def test_repro_tilelang_bucketed_dkv_collision():
    run_repro(mode="collision", repeat=3, collision_keys=2)
...
if __name__ == "__main__":
    ...
    if not _env_enabled():
        missing = ", ".join(f"{name}=1" for name in _REQUIRED_ENV if os.getenv(name, "0").lower() not in {"1", "true", "yes", "on"})
        raise SystemExit(f"Missing deterministic env var(s): {missing}")
    if paddle.is_compiled_with_cuda():
