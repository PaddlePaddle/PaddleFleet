...
本文旨在说明如何基于 PaddleFleet 进行模型的预训练（后预训练流程可以直接参考本文档）。
...
PaddleFleet 支持 cli 直接启动训练，如下示例：

#### 全量训练

```shell
CUDA_VISIBLE_DEVICES=0 paddlefleet-cli train pt_full.yaml
```

```shell
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 paddlefleet-cli train pt_full.yaml
```
