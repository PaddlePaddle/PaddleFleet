...
本文旨在说明如何基于 PaddleFleet 进行模型的指令微调。
...
```shell
CUDA_VISIBLE_DEVICES=0 paddlefleet-cli train sft_full.yaml
```

```shell
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 paddlefleet-cli train sft_full.yaml
```

#### lora 训练

```shell
CUDA_VISIBLE_DEVICES=0 paddlefleet-cli train sft_lora.yaml
```

```shell
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 paddlefleet-cli train sft_lora.yaml
```
...
```shell
paddlefleet-cli export run_export.yaml
```
...
```python
import paddle
from paddlefleet.transformers import AutoModelForCausalLM, AutoConfig, AutoTokenizer
