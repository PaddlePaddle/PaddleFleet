...
当前 PaddleFleet 的预训练、后训练数据流支持 `jsonl` 、 `json` 等格式的数据，训练时需确保文件名后缀和文件内容格式保持一致。

## 1.1. 新增文件格式支持

如果您有格外的文件格式需要支持，可以在 `paddlefleet/datasets/reader/io.py` 里面实现各种类型文件的读取函数，返回的数据格式为 `List（Dict）`
...
然后在 `paddlefleet/datasets/reader/file_reader.py` 中 `BaseReader` 的 `self.loader_map` 中进行注册：
...
PaddleFleet 支持业界常用的不同的数据集格式，通常情况下，我们推荐使用 `messages` 格式，该格式下数据的表示较为直观且功能齐全。
...
```bash
CUDA_VISIBLE_DEVICES=0 paddlefleet-cli train examples/config/sft/full.yaml make_offline_data=true
```
...
PaddleFleet 支持多种不同的数据格式处理，我们通过将其他数据格式转换为 messages 格式来实现该功能

如果您有其他需要支持的数据格式，可以在 `paddlefleet/datasets/reader/convertor.py` 里面实现对应格式的转换函数，统一转换成 messages 格式。
...
然后在 `paddlefleet/datasets/reader/file_reader.py` 中 `BaseReader` 的 `self.convertor_map` 中进行注册：
