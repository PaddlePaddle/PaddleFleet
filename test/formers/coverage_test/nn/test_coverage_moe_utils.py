...
class TestGetHCG(unittest.TestCase):
    ...
    def setUp(self):
        from paddlefleet.nn.moe.utils import get_hcg

        self.get_hcg = get_hcg

    @patch("paddlefleet.nn.moe.utils.fleet.get_hybrid_communicate_group")
...
class TestScatterAxis(unittest.TestCase):
    ...
    def setUp(self):
        from paddlefleet.nn.moe.utils import scatter_axis
    ...
    def test_default_group_uses_model_parallel(self):
        ...
        x = _randn([8, 4])
        with patch(
            "paddlefleet.nn.moe.utils.fleet.get_hybrid_communicate_group"
        ) as mock_get:
            mock_hcg = MagicMock()
...
class TestDetachAndRequiresGrad(unittest.TestCase):
    ...
    def setUp(self):
        from paddlefleet.nn.moe.utils import detach_and_requires_grad_
...
class TestFakeClone(unittest.TestCase):
    ...
    def setUp(self):
        from paddlefleet.nn.moe.utils import FakeClone
...
class TestReduceScatterGroupOp(unittest.TestCase):
    ...
    def setUp(self):
        from paddlefleet.nn.moe.utils import ReduceScatterGroupOp

        self.ReduceScatterGroupOp = ReduceScatterGroupOp

    @patch("paddlefleet.nn.moe.utils.reduce_scatter_group")
    ...
    @patch("paddlefleet.nn.moe.utils.all_gather_group")
    @patch("paddlefleet.nn.moe.utils.reduce_scatter_group")
...
class TestAllGatherGroupOp(unittest.TestCase):
    ...
    def setUp(self):
        from paddlefleet.nn.moe.utils import AllGatherGroupOp
...
class TestAllGatherGroup(unittest.TestCase):
    ...
    def setUp(self):
        from paddlefleet.nn.moe.utils import all_gather_group
    ...
    @patch("paddlefleet.nn.moe.utils.dist.stream.all_gather")
...
class TestReduceScatterGroup(unittest.TestCase):
    ...
    def setUp(self):
        from paddlefleet.nn.moe.utils import reduce_scatter_group
    ...
    @patch("paddlefleet.nn.moe.utils.dist.stream.reduce_scatter")
...
class TestScatterOp(unittest.TestCase):
    ...
    def setUp(self):
        from paddlefleet.nn.moe.utils import ScatterOp

        self.ScatterOp = ScatterOp

    @patch("paddlefleet.nn.moe.utils.scatter_axis")
...
class TestParseMoeGroup(unittest.TestCase):
    ...
    def setUp(self):
        from paddlefleet.nn.moe.utils import _parse_moe_group
    ...
    def test_world_group(self):
        ...
        with patch("paddlefleet.nn.moe.utils._get_global_group") as mock_global:
            mock_group = _make_fake_group()
...
class TestGetAsyncLoader(unittest.TestCase):
    ...
    def setUp(self):
        import paddlefleet.nn.moe.utils as utils_module
        from paddlefleet.nn.moe.utils import get_async_loader
    ...
    def test_with_hcg_creates_loader(self):
        ...
        with patch(
            "paddlefleet.nn.moe.utils.fleet.fleet", new_callable=PropertyMock
        ) as _:
            mock_f = MagicMock()
            mock_f._hcg = True
            ...
            with patch("paddlefleet.nn.moe.utils.get_hcg") as mock_get_hcg:
                mock_get_hcg.return_value = mock_hcg
                with patch(
                    "paddlefleet.nn.moe.utils.create_async_load"
                ) as mock_create:
...
class TestManualBackward(unittest.TestCase):
    ...
    def setUp(self):
        from paddlefleet.nn.moe.utils import manual_backward
