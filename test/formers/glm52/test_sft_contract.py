...
from paddlefleet.cli.train.sft import workflow as sft_workflow
...
def test_load_tokenizer_uses_independent_source():
    ...
    with (
        ...
        actual_tokenizer, actual_processor = load_tokenizer_and_processor(
...
def test_glm52_provider_selects_numeric_contract_without_changing_glm4_default():
    ...
    assert not GlmMoeDsaModelProvider(
        **values, defer_token_normalization=False
...
def test_native_numerator_reporting_requires_deferred_normalization(
    ...
    trainer = SimpleNamespace(
        model=SimpleNamespace(
            config=SimpleNamespace(
                use_accuracy_compatible=enabled,
                defer_token_normalization=deferred,
            )
        ),
    ...
    assert (
        sft_workflow.SFTTrainer._requires_native_token_weighted_logging(trainer)
        is expected
    )
...
def test_glm_moe_dsa_training_contract_copies_pp_p2p_from_training_args():
    ...
    training_args = _base_training_args(
        overlap_p2p_comm=False, batch_p2p_comm=True
    )
    model_args = SimpleNamespace(
        mtp_attention_flexible=True, persist_layer_norm=False
    )
...
def test_glm_moe_dsa_training_contract_uses_yaml_despite_legacy_env(
    monkeypatch, enabled
):
    model_config = SimpleNamespace(
        model_type="glm_moe_dsa", use_accuracy_compatible=not enabled
    )
    training_args = _base_training_args(
        use_accuracy_compatible=enabled,
    ...
    apply_glm_moe_dsa_training_contract(
        model_config, training_args, model_args, SimpleNamespace()
    )

    assert model_config.use_accuracy_compatible == (
        "megatron" if enabled else False
    )
...
def test_glm_moe_dsa_training_contract_preserves_normalized_accuracy_target(
    requested, expected
):
    model_config = SimpleNamespace(model_type="glm_moe_dsa")
...
def test_glm_moe_dsa_training_contract_keeps_registered_mtp_loss_weight_when_cli_is_silent():
    model_config = SimpleNamespace(
        model_type="glm_moe_dsa", mtp_loss_scaling_factor=0.1
    )
    training_args = _base_training_args()
    model_args = SimpleNamespace(
        mtp_attention_flexible=True, persist_layer_norm=False
...
def test_glm_moe_dsa_training_contract_preserves_unspecified_fusion_default(
    monkeypatch,
):
    model_config = SimpleNamespace(
        model_type="glm_moe_dsa", bias_activation_fusion=True
    )
