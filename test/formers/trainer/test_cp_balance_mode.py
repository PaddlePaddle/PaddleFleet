...
class TestGetInputsListCpBalanceMode(unittest.TestCase):
    ...
    def test_passes_contiguous_mode(self):
        ...
        with (
            patch(
                "paddlefleet.trainer.trainer.is_paddlefleet_available",
                return_value=True,
            ),
            patch("paddlefleet.trainer.trainer.FleetGPTModel", str),
            patch(
                "paddlefleet.trainer.trainer.get_batch_on_this_cp_rank",
                mock_get_batch,
            ),
        ):
