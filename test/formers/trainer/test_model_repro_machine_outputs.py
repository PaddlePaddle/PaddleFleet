...
SOURCE = ROOT / "src/paddlefleet/cli/train/sft/workflow.py"
tree = ast.parse(SOURCE.read_text())
callback = next(
    n
    for n in tree.body
    if isinstance(n, ast.ClassDef) and n.name == "ModelReproObservationCallback"
)
selected = {
    "__init__",
    "_is_writer",
...
exec(
    compile(
        ast.fix_missing_locations(ast.Module(body=[callback], type_ignores=[])),
        str(SOURCE),
        "exec",
    ),
    namespace,
...
class MachineOutputTests(unittest.TestCase):
    def test_unrounded_main_loss_and_stale_window_replacement(self):
        with tempfile.TemporaryDirectory() as directory:
            raw, loss = (
                ...
                Path(directory) / "loss.json",
            )
            raw.write_text("old invocation\n")
            loss.write_text("old invocation\n")
            with patch.dict(
                os.environ, {"MODEL_REPRO_LOSS_PATH": str(loss)}, clear=True
            ):
                ...
                callback.on_log(
                    ...
                    state,
                    None,
                    logs={"loss": round(value, 8), "mtp 1 loss": 0.5},
                    raw_loss=value,
                )
    ...
    def test_loss_without_raw_path_and_non_writer_is_inert(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.dict(
                ...
                callback = Callback()
                state = SimpleNamespace(
                    is_world_process_zero=False, global_step=1
                )
                callback.on_train_begin(None, state, None)
    ...
    def test_environment_written_only_after_train_begin(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.dict(
                os.environ, {"MODEL_REPRO_ENV_PATH": str(env)}, clear=True
            ):
                callback = Callback(
                    model_source="loaded/model", weights_loaded=True
                )
                ...
                callback.on_train_begin(
                    None, SimpleNamespace(is_world_process_zero=True), None
                )
                self.assertEqual(
                    json.loads(env.read_text()),
                    {"weights_loaded": True, "model_source": "loaded/model"},
                )
