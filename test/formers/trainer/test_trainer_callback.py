...
from paddlefleet.trainer.trainer import DEFAULT_CALLBACKS
...
class TrainerCallbackTest(unittest.TestCase):
    ...
    def test_internal_medicine_does_not_gather_unsampled_step(self):
        ...
        callback._monitor_dict = {
            "monitor": SimpleNamespace(sampled_this_step=False)
        }
    ...
    def test_event_flow(self):
        ...
        trainer = self.get_trainer(
            callbacks=[MyTestTrainerCallback], logging_steps=500, save_steps=5
        )
        trainer.train()
        events = trainer.callback_handler.callbacks[-2].events
        self.assertEqual(events, self.get_expected_events(trainer))
        ...
        with patch("paddlefleet.trainer.logger.warning") as warn_mock:
            trainer = self.get_trainer(
                logging_steps=500,
