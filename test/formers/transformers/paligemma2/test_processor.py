...
CHECKPOINT = os.path.abspath(
    os.path.join(
        os.path.dirname(__file__), "../../../..", "paligemma2-3b-pt-448"
    )
)
...
class PaliGemmaProcessorTest(unittest.TestCase):
    ...
    def test_processor_outputs_expected_multimodal_inputs(self):
        ...
        self.assertEqual(
            int((inputs["input_ids"] == self.processor.image_token_id).sum()),
            1024,
        )
    ...
    def test_auto_processor_resolves_paligemma_processor_from_preprocessor_config(
        self,
    ):
        with tempfile.TemporaryDirectory() as tempdir:
            with open(
                os.path.join(tempdir, "preprocessor_config.json"),
                "w",
                encoding="utf-8",
            ) as config_file:
                json.dump(
                    {"processor_class": "PaliGemmaProcessor"}, config_file
                )
            ...
            with patch.object(
                ...
                processor = AutoProcessor.from_pretrained(
                    tempdir, download_hub="huggingface", local_files_only=True
                )
    ...
    @unittest.skipUnless(
        ...
        "requires local paligemma2-3b-pt-448 checkpoint",
    )
    def test_processor_real_checkpoint(self):
        ...
        self.assertEqual(
            int((inputs["input_ids"] == processor.image_token_id).sum()), 1024
        )
