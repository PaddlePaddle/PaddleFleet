...
method = next(
    ...
    if isinstance(n, ast.FunctionDef) and n.name == "_gen_inv_aoa_config"
)
method.decorator_list = []
namespace = {
    "copy": copy,
    "MoEAOAConfigGenerator": base.MoEAOAConfigGenerator,
    "GlmMoeDsaConfig": object,
}
exec(
    compile(
...
class ProviderExportMappingTests(unittest.TestCase):
    def configuration(self, native):
        ...
        setattr(
            config,
            "dsa_indexer_types" if native else "indexer_types",
    ...
    def test_grouped_expert_intermediates_have_concrete_rules(self):
        statements = export_config(None, self.configuration(True))[
            "aoa_statements"
        ]
        self.assertFalse(
            any("$EXPERT_ID" in statement for statement in statements)
        )
        for expert in (0, 15):
            self.assertIn(
