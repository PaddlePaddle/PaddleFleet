...
from paddlefleet.transformers import Qwen3Config
from paddlefleet.transformers.configuration_utils import (
    PretrainedConfig,
    attribute_map,
)
from paddlefleet.transformers.model_utils import PretrainedModel
from paddlefleet.utils import CONFIG_NAME
from paddlefleet.utils.env import LEGACY_CONFIG_NAME
...
class ConfigurationUtilsTest(unittest.TestCase):
    ...
    def test_model_config_save(self):
        ...
        with tempfile.TemporaryDirectory() as tp:
            ...
            assert "paddlefleet_version" in loaded_config, "always save paddlefleet_version"
            assert (
...
class StandardConfigMappingTest(unittest.TestCase):
    def test_qwen3_config_mapping(self):
        ...
        config = FakeQwen3Config.from_pretrained("PaddleFleet/tiny-random-qwen3")
        hidden_size = config.hidden_size

        FakeQwen3Config.attribute_map = {"fake_field": "hidden_size"}

        loaded_config = FakeQwen3Config.from_pretrained("PaddleFleet/tiny-random-qwen3")
