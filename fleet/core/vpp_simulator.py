...
class VPPSimulator:
    ...
    def _add_bubble(self):
        ...
        while num_done_chunks < num_chunks:
            ...
            if micro_step < num_chunks_each_stage:
                ...
                if micro_step > 0:
                    chunk.start = max(chunk.start, self.schedule_table[stage_index][micro_step - 1].end)
    ...
    def _barrier(self):
        ...
        for micro_step in range(last_stage_warmup_steps, last_stage_warmup_steps + 2 * last_stage_steady_steps):
        ...
        for stage_id in range(0, self.pp_degree - 1):
            for micro_step, chunk in enumerate(self.schedule_table[stage_id]):
                if micro_step > 0:
                    chunk.start = max(chunk.start, self.schedule_table[stage_id][micro_step - 1].end)
                    chunk.end = chunk.start + self._get_consume_time(
                        chunk.virtual_pp_rank, chunk.acc_step, chunk.chunk_type
                    )
    ...
    def draw_chunks(self):
        ...
        for stage_idx, chunks in enumerate(self.schedule_table):
            y_pos = (self.pp_degree - stage_idx - 1) * stage_height  # Y position corresponds to stage_id, adjusted by stage_height
            for chunk in chunks:
                if chunk.chunk_type == ChunkType.BUBBLE:
                    continue
                x = chunk.start
                ...
                rect = patches.Rectangle(
                    (x, y_pos), width, stage_height, linewidth=1, edgecolor="black", facecolor=color
                )
                ax.add_patch(rect)
                ...
                ax.text(text_x, text_y, text, ha="center", va="center", color="white", fontsize=8)
    ...
    def draw_balls(self):
        ...
        for j in range(self.num_acc_steps - 1):
            for i in range(self.layer_num):
                ...
                # Draw line segment
                ax.plot(
                    [current_pos[0], next_pos[0]], [current_pos[1], next_pos[1]], color="gray", linewidth=1, zorder=1
                )

        # Draw balls with zorder to ensure they are on top layer
        for j in range(self.num_acc_steps):
            for i in range(self.layer_num):
                circle = plt.Circle((j, i), 0.2, edgecolor="black", facecolor="lightblue", zorder=2)
                ax.add_patch(circle)
