...
// ==========================================================================
// Clamped variants — separate device functions and kernels with clamp_value
// Non-clamp path uses original device functions/kernels above (zero overhead)
// ==========================================================================
...
#define LAUNCH_FUSED_SPAQ_CLAMP(__using_pow2_scaling, __with_prob)         \
  do {                                                                     \
    auto kernel = FusedSPAQKernelClamp<__using_pow2_scaling,               \
                                       __with_prob,                        \
                                       ScaleT,                             \
                                       use_ue8m0>;                         \
    kernel<<<grid, block, 0, stream>>>(                                    \
...
__global__ void FusedSPAQKernelVec4Clamp(const phi::bfloat16* __restrict__ Xin,
  ...
  for (int64_t base_y = blockIdx.y; base_y < rows; base_y += gridDim.y) {
    ...
    act_f32x4 = fast_swiglu_vec4_clamp(lhs_bf16x4, rhs_bf16x4, clamp_value);
  ...
  if (cols % 16 == 0) {
    ...
    DISPATCH_BOOL(
        ...
        DISPATCH_BOOL(
            ...
            grid.x =
                ((cols / 2) + block.x * vec_numel - 1) / (block.x * vec_numel);
            LAUNCH_FUSED_SPAQ_VEC8_CLAMP(k_using_pow2_scaling, k_with_prob);))
  } else if (cols % 8 == 0) {
    block.x = thread_per_block;
    constexpr int vec_numel = 4;
    const int scale_cols = (cols / 2 + 127) / 128;
    DISPATCH_BOOL(
        ...
        DISPATCH_BOOL(
            ...
            grid.x =
                ((cols / 2) + block.x * vec_numel - 1) / (block.x * vec_numel);
            LAUNCH_FUSED_SPAQ_VEC4_CLAMP(k_using_pow2_scaling, k_with_prob);))
  } else {
    block.x = thread_per_block;
    DISPATCH_BOOL(
        using_pow2_scaling,
        ...
        DISPATCH_BOOL(
            with_prob, k_with_prob, grid.y = rows > 65535 ? 65535 : rows;
            grid.x = ((cols / 2) + block.x - 1) / block.x;
            LAUNCH_FUSED_SPAQ_CLAMP(k_using_pow2_scaling, k_with_prob);))
  }
}
