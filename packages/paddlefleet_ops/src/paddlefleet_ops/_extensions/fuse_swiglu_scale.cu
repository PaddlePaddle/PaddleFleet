...
#pragma unroll
  ...
  static __shared__ float shared_sum[256];
  if (tid < 256) shared_sum[tid] = local_d_probs_sum;
  __syncthreads();
...
static std::vector<paddle::Tensor> FusedSwiGLUWeightedBackwardImpl(
  ...
  // Paddle extension gridDim is int, so rows must be <= INT_MAX
  PADDLE_ENFORCE_LE(
      rows,
      static_cast<int64_t>(std::numeric_limits<int>::max()),
      common::errors::InvalidArgument(
          "rows must be <= INT_MAX for fused_swiglu_weighted_bwd."));
...
std::vector<paddle::Tensor> FusedSwiGLUWeightedBackward(
    const paddle::Tensor& x,
    const paddle::Tensor& probs,
    const paddle::Tensor& d_out) {
  return FusedSwiGLUWeightedBackwardImpl</*kHasClamp=*/false>(
      x, probs, d_out, 0.0);
}
...
// Non-clamp version: replaces external FusedQuantOps._fused_swiglu_probs_bwd
PD_BUILD_OP(fused_swiglu_weighted_bwd)
    .Inputs({"X", "Probs", "DOut"})
    .Outputs({"DX", "DProbs", "Out"})
    .SetKernelFn(PD_KERNEL(FusedSwiGLUWeightedBackward))
