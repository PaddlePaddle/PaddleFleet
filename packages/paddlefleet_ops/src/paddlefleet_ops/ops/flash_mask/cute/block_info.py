...
class BlockInfo:
    ...
    def get_n_block_min_max(
        ...
        if const_expr(
            self.is_causal
            or (self.is_local and self.window_size_right is not None)
        ):
            m_idx_max = (m_block + 1) * self.tile_m
            if const_expr(self.qhead_per_kvhead_packgqa > 1):
                m_idx_max = cute.ceil_div(
                    m_idx_max, self.qhead_per_kvhead_packgqa
                )
            ...
            n_block_max = min(
                n_block_max, cute.ceil_div(n_idx_right, self.tile_n)
            )
        n_block_min = 0
        ...
        if cutlass.const_expr(self.is_split_kv):
            ...
            n_block_max = cutlass.min(
                n_block_min + num_n_blocks_per_split, n_block_max
            )
    ...
    def get_n_block_min_before_local_mask(
        ...
        if const_expr(not self.is_local or self.window_size_left is None):
            ...
            n_idx_left = n_idx - self.window_size_left
            return cutlass.max(
                n_block_min, cute.ceil_div(n_idx_left, self.tile_n)
            )
