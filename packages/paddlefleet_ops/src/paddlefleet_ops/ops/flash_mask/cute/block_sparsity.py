...
def _expand_sparsity_tensor(
    ...
    can_expand = all(
        map(
            lambda cur, tgt: cur == tgt or cur == 1,
            tensor.shape,
            expected_shape,
        )
    )
...
def is_block_sparsity_enabled(tensors: BlockSparseTensorsPaddle) -> bool:
    return any(
        t is not None for t in (tensors.full_block_cnt, tensors.mask_block_cnt)
    )
...
def _compute_sparsity(
    config: Config, device: str, aux_tensors: list[paddle.Tensor] | None
    ...
    # --- Block Causal Mask ---
        ...
        for q_block_idx in range(n_blocks_q):
            ...
            if num_causal_indices > 0:
                ...
                full_block_idx[:, :, q_block_idx, :num_causal_indices] = (
                    causal_indices
                )
    ...
    # --- Causal and Sliding Window Masks ---
        ...
        for q_block_idx in range(n_blocks_q):
            ...
            if len(full_indices) > 0:
                ...
                full_block_idx[:, :, q_block_idx, : len(full_indices)] = (
                    full_indices
                )
...
def _compute_varlen_sparsity(
    ...
    assert cu_seqlens_k is not None, (
        "cu_seqlens_k is required for varlen attention"
    )
    ...
    **kwargs,
    ...
    for m_local in range(n_blocks_q):
        ...
        if full_blocks:
            ...
            full_block_idx[seq_idx, :, m_local, : len(full_blocks)] = (
                paddle.to_tensor(
                    full_blocks,
                )
            )
        if partial_blocks:
            ...
            mask_block_idx[seq_idx, :, m_local, : len(partial_blocks)] = (
                paddle.tensor(
                    partial_blocks,
                )
            )
...
def _compute_sliding_window_varlen_blocks(
    ...
    **kwargs,
    ...
    is_full_fn = lambda m_start, m_end, n_start, n_end: (
        n_end - 1 <= m_start
    ) and (n_start >= m_start - window_size + 1)
        ...
        for m_local in range(n_blocks_q):
            ...
            for n_local in range(n_blocks_k):
                ...
                unmasked_count = sum(
                    ...
                    if mask_mod_flex(
                        seq_idx, h_q, q_pos, k_pos, seq_len_q, seq_len_k
                    )
                )
