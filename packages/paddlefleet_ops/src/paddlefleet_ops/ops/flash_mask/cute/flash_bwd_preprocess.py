...
from flash_mask.cute import copy_utils, utils
...
class FlashAttentionBackwardPreprocess:
    def __init__(
        ...
        if dq_head_dim is not None:
            self.dq_head_dim_padded = int(
                math.ceil(dq_head_dim / hdim_multiple_of) * hdim_multiple_of
            )
        else:
    ...
    def kernel(
        ...
        if work_tile.is_valid_tile:
            ...
            if cutlass.const_expr(not seqlen.has_cu_seqlens_q):
                ...
                padded_offset_q = (
                    seqlen.offset_q + batch_size * self.m_block_size
                )
            ...
            dpsum = utils.warp_reduce(
                dpsum, operator.add, width=threads_per_row
            )
            ...
            if cutlass.const_expr(mdQaccum is not None):
                if cutlass.const_expr(not seqlen.has_cu_seqlens_q):
                    ...
                    mdQaccum_cur = cute.domain_offset(
                        (padded_offset_q * self.dq_head_dim_padded,),
                ...
                blkdQaccum_shape = (
                    self.m_block_size * self.dq_head_dim_padded,
                )
            ...
            if cutlass.const_expr(mLSE is not None):
                ...
                if tidx < seqlen_q_rounded - m_block * self.m_block_size:
                    gLSElog2[tidx] = (
                        lse * LOG2_E if lse != -Float32.inf else 0.0
                    )
