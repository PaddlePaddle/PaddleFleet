...
from flash_mask.cute import ampere_helpers as sm80_utils, utils
...
class FlashAttentionBackwardSm80:
    ...
    def _check_type(
        ...
        if cutlass.const_expr(self.qhead_per_kvhead == 1):
            ...
            if cutlass.const_expr(
                not (mdK_type == mdV_type == cutlass.Float32)
            ):
                raise TypeError(
                    "mdKaccum and mdVaccum tensors must have the data type Float32"
                )
        if cutlass.const_expr(
            mQ_type not in [cutlass.Float16, cutlass.BFloat16]
        ):
    ...
    def _get_shared_storage_cls(self):
        ...
        cosize_sQV = max(
            cute.cosize(self.sQ_layout), cute.cosize(self.sV_layout)
        )
        ...
        if work_tile.is_valid_tile:
            ...
            if cutlass.const_expr(not seqlen.has_cu_seqlens_q):
                ...
                mLSE_cur = cute.domain_offset(
                    (padded_offset_q,), mLSE[head_idx, None]
                )
            ...
            if cutlass.const_expr(not seqlen.has_cu_seqlens_k):
                ...
                mK_cur, mV_cur = [
                    cute.domain_offset(
                        (seqlen.offset_k, 0), t[None, head_idx_kv, None]
                    )
                    for t in (mK, mV)
                ]
            ...
            tdQrdS = utils.mma_make_fragment_A(
                sdS, thr_mma_dq, swapAB=self.dQ_swapAB
            )
            tdQrK = utils.mma_make_fragment_B(
            ...
            tSsLSEMma = utils.make_acc_tensor_mn_view(
    ...
    def compute_one_m_block(
        ...
        tLSErdPsum = cute.make_fragment_like(
        ...
        for r in cutlass.range(
            cute.size(acc_dP_mn, mode=[0]), unroll_full=True
        ):
        ...
        if cutlass.const_expr(not self.Mma_dKV_is_RS):
            ...
            cute.copy(
                smem_copy_params.r2s_thr_copy_PdS, tPrP, smem_copy_params.tPsP
            )
        ...
        if cutlass.const_expr(not self.Mma_dKV_is_RS):
            ...
            cute.copy(
                smem_copy_params.r2s_thr_copy_PdS,
                tdSrdS,
                smem_copy_params.tdSsdS,
            )
        ...
        def dQ_mma(hook_fn):
            ...
            # ((1, 1), num_elements)
            acc_dQ_atomic = gmem_copy_params.gmem_thr_copy_dQaccum.retile(
                acc_dQ
            )
            tdQgdQaccum_atomic = gmem_copy_params.tdQgdQaccum[
                None, None, m_block
            ]
    ...
    def epilogue(
        ...
        if cutlass.const_expr(self.qhead_per_kvhead == 1):
            ...
            if cutlass.const_expr(not seqlen.has_cu_seqlens_k):
                mdK_cur, mdV_cur = [
                    t[batch_idx, head_idx_kv, None] for t in (mdK, mdV)
                ]
            else:
                padded_offset_k = (
                    seqlen.offset_k + batch_idx * self.n_block_size
                )
                mdK_cur = cute.domain_offset(
                    (padded_offset_k * self.head_dim_padded,),
                    mdK[head_idx_kv, None],
                )
                mdV_cur = cute.domain_offset(
                    (padded_offset_k * self.head_dim_v_padded,),
                    mdV[head_idx_kv, None],
                )
    ...
    def load_K(
        ...
        for n in cutlass.range_constexpr(cute.size(tKsK.shape[1])):
            ...
            if (
                ...
                predicate_n = (
                    t0KcK[0, n, 0][0]
                    < seqlen - block * self.n_block_size - tKcK[0][0]
                )
    ...
    def load_V(
        ...
        for n in cutlass.range_constexpr(cute.size(tVsV.shape[1])):
            ...
            if (
                ...
                predicate_n = (
                    t0VcV[0, n, 0][0]
                    < seqlen - block * self.n_block_size - tVcV[0][0]
                )
    ...
    def load_Q_LSE(
        ...
        for m in cutlass.range_constexpr(cute.size(tQsQ.shape[1])):
            ...
            if (
                ...
                predicate_m = (
                    t0QcQ[0, m, 0][0]
                    < seqlen - block * self.m_block_size - tQcQ[0][0]
                )
