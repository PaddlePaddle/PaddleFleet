...
def create_softcap_scoremod(softcap_val):
    ...
    def scoremod_premask_fn(
        acc_S_SSA, batch_idx, head_idx, q_idx, kv_idx, buffers
    ):
...
def convert_from_dlpack(
    x, leading_dim, alignment=16, divisibility=1
) -> cute.Tensor:
    return (
...
def convert_from_dlpack_leading_static(
    ...
    for i in range(x.ndim):
        if i != leading_dim and (static_modes is None or i not in static_modes):
            x_ = x_.mark_compact_shape_dynamic(
                mode=i, stride_order=stride_order
            )
    return x_
...
def fadd_reduce(
    ...
    if const_expr(arch < 100 or cute.size(x.shape) % 8 != 0):
        ...
        for i in cutlass.range_constexpr(8, cute.size(x.shape), 8):
            local_sum[0] = add_packed_f32x2(
                local_sum[0], (res[i + 0], res[i + 1])
            )
            local_sum[1] = add_packed_f32x2(
                local_sum[1], (res[i + 2], res[i + 3])
            )
            local_sum[2] = add_packed_f32x2(
                local_sum[2], (res[i + 4], res[i + 5])
            )
            local_sum[3] = add_packed_f32x2(
...
def e2e_asm2(
    ...
    out_f32x2 = llvm.inline_asm(
        ...
        [
            Float32(x).ir_value(loc=loc, ip=ip),
            Float32(y, loc=loc, ip=ip).ir_value(),
        ],
        "{\n\t"
...
def coord_offset_i64(
    ...
    new_layout = cute.slice_(
        tensor.layout,
        (*[None] * dim, 0, *[None] * (cute.rank(tensor) - dim - 1)),
    )
