...
def mask_r2p(
    ...
    ncol = const_expr(
        cute.size(X.shape[cute.rank(X) - 1])
        if not rank1
        else cute.size(X.shape)
    )
...
def mask_r2p_transposed(
    X: cute.Tensor, row_limit_top: Int32, num_rep: int
) -> None:
    # Bit manipulation, compiles down to the R2P instruction
    ...
    num_wg = 2
    row_limit_top_transformed = row_limit_top // (
        num_rep * num_wg
    ) * num_rep + min(row_limit_top % (num_rep * num_wg), num_rep)
...
class AttentionMask:
    ...
    def apply_mask(
        ...
        if n_block < 0:
            n_block = 0
        seqlenk_col_limit = (
            self.seqlen_k - n_block * self.tile_n - thr_col_offset
        )
        if const_expr(not mask_causal and not mask_local and mask_mod is None):
            if const_expr(mask_seqlen):
                # The compiler now chooses not to use R2P
                ...
                if const_expr(not r2p):
                    ...
                    for c in cutlass.range(
                        ...
                        for r in cutlass.range(
                            ...
                            acc_S_mn[r, c] = (
                                -Float32.inf if oob else acc_S_mn[r, c]
                            )
                else:
            ...
            wrap_aux_indices = const_expr(
                has_fastdiv
                and mask_seqlen
                and const_expr(aux_tensors is not None)
            )
            ...
            for r in cutlass.range_constexpr(nrow):
                ...
                for col in cutlass.range_constexpr(ncol):
                    ...
                    batch_idx_ssa = utils.scalar_to_ssa(
                        batch_idx, cutlass.Int32
                    )
            ...
            if const_expr(not self.swap_AB):
                ...
                if const_expr(self.qhead_per_kvhead_packgqa != 1):
                    assert not self.swap_AB, (
                        "swap_AB with PackGQA not supported yet"
                    )
    ...
    def apply_mask_sm100(
        ...
        kv_stage: Int32 = None,
        stage: Int32 = None,
        load_startend_row_indices_consumer_state: cutlass.pipeline.PipelineState
        | None = None,
        ...
        if const_expr(not mask_causal and not mask_local and mask_mod is None):
            if const_expr(mask_seqlen):
                if const_expr(not r2p):
                    for i in cutlass.range(
                        cute.size(tScS_t2r.shape), unroll_full=True
                    ):
            ...
            for i in cutlass.range_constexpr(ncol):
                col_coord = (
                    tScS_t2r[i][1] if not self.swap_AB else tScS_t2r[i][0]
                )
        ...
        if const_expr(enable_flashmask):
            ...
            cute.arch.mbarrier_wait(
                mbar_ptr
                + mbar_load_startend_row_indices_full_offset
                + stage * kv_stage
                + load_startend_row_indices_consumer_state.index,
                load_startend_row_indices_consumer_state.phase,
            )

            if (
                n_block_idx < generate_block_buffer_usable_block_count
                and encode_n_block >= 0
            ):
                if const_expr(has_ut_start):
                    for i in cutlass.range(
                        ...
                        if (tScS_t2r[i][0] >= lts and tScS_t2r[i][0] < lte) or (
                            tScS_t2r[i][0] >= uts and tScS_t2r[i][0] < ute
                        ):
            ...
            cute.arch.mbarrier_arrive(
                mbar_ptr
                + mbar_load_startend_row_indices_empty_offset
                + stage * kv_stage
                + load_startend_row_indices_consumer_state.index
            )
        ...
        if const_expr(not mask_causal and not mask_local):
            ...
            if partially_masked:
                for i in cutlass.range(
                    ...
                    lts = (
                        sStartEndRowIndices[tScS_t2r[i][COL], 0]
                        - m_block * self.tile_m
                    )
                    ute = (
                        sStartEndRowIndices[tScS_t2r[i][COL], 1]
                        - m_block * self.tile_m
                    )
                    acc_S[i] = (
                        -cutlass.Float32.inf
                        if tScS_t2r[i][ROW] >= lts
                        else acc_S[i]
                    )
                    acc_S[i] = (
                        -cutlass.Float32.inf
                        if tScS_t2r[i][ROW] < ute
                        else acc_S[i]
                    )
                ...
                if partially_masked:
                    ...
                    for i in cutlass.range(
                        ...
                        acc_S[i] = (
                            -cutlass.Float32.inf
                            if tScS_t2r[i][ROW] >= lts
                            and tScS_t2r[i][ROW] < lte
                            else acc_S[i]
                        )
            else:
                raise AssertionError("Local masking isn't supported yet")
