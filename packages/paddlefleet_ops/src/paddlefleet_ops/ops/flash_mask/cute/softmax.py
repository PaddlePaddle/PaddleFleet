...
class Softmax(ParamsBase):
    ...
    def online_softmax(
        ...
        for r in cutlass.range(cute.size(row_max), unroll_full=True):
            ...
            row_max_cur = utils.warp_reduce(
                row_max_cur, cute.arch.fmax, width=4
            )
            if cutlass.const_expr(check_inf):
                row_max_cur = (
                    0.0 if row_max_cur == -Float32.inf else row_max_cur
                )
...
class SoftmaxSm100(Softmax):
    ...
    def update_row_sum(
        ...
        init_val = (
            self.row_sum[0] * row_scale
            if cutlass.const_expr(not is_first)
            else None
        )
        # self.row_sum[0] = self._compute_row_sum(acc_S_row_exp, init_val=self.row_sum[0] * row_scale)
        self.row_sum[0] = self._compute_row_sum(
            acc_S_row_exp, init_val=init_val
        )
    ...
    def scale_subtract_rowmax(
        ...
        for i in cutlass.range(
            0, cute.size(acc_S_row.shape), 2, unroll_full=True
        ):
    ...
    def apply_exp2_convert(
        ...
        acc_S_row_frg = cute.logical_divide(
            acc_S_row, cute.make_layout(frg_tile)
        )
        ...
        for j in cutlass.range_constexpr(frg_cnt):
            for k in cutlass.range_constexpr(
                0, cute.size(acc_S_row_frg, mode=[0]), 2
            ):
                ...
                if cutlass.const_expr(not e2e):
                    ...
                    if cutlass.const_expr(
                        ...
                        acc_S_row_frg[k, j] = cute.arch.exp2(
                            acc_S_row_frg[k, j]
                        )
                        acc_S_row_frg[k + 1, j] = cute.arch.exp2(
                            acc_S_row_frg[k + 1, j]
...
def apply_score_mod_inner(
    ...
    for i in cutlass.range(0, n_vals, vec_size, unroll_full=True):
        for j in cutlass.range(vec_size, unroll_full=True):
            ...
            if cutlass.const_expr(
                qhead_per_kvhead > 1 and constant_q_idx is None
            ):
            ...
            if cutlass.const_expr(
                ...
                _, kv_idx_wrapped = divmod(
                    index_tensor[i + j][1], seqlen_k_divmod
                )
