...
from flash_mask.cute.hopper_helpers import gemm_w_idx, gemm_zero_init
from flash_mask.cute.mask import AttentionMask
from flash_mask.cute.named_barrier import NamedBarrierBwd, NamedBarrierFwd
...
class FlashAttentionBackwardSm90:
    ...
    def _setup_attributes(self):
        (
            ...
            sm90_utils.make_smem_layout(
                self.dtype, LayoutEnum.ROW_MAJOR, shape, stage
            )
            for shape, stage in [
    ...
    def _get_tiled_mma(self):
        ...
        tiled_mma_dK, tiled_mma_dV = [
            sm90_utils_basic.make_trivial_tiled_mma(
                ...
                atom_layout_mnk=(
                    atom_layout_dKV
                    if not self.dKV_swapAB
                    else atom_layout_dKV[::-1]
                )
                + (1,),
                tiler_mn=tiler_mn_d
                if not self.dKV_swapAB
    ...
    def _get_shared_storage_cls(self):
        sQ_alignment = sK_alignment = sV_alignment = sdQaccum_alignment = (
            sdO_alignment
        ) = 1024
        ...
        sQ_struct, sK_struct, sV_struct, sdO_struct, sdQaccum_struct = [
            ...
            for (layout, type, alignment) in [
                ...
                (self.sV_layout, self.dtype, sV_alignment),
        ...
        class SharedStorageQKV:
            ...
            sP: cute.struct.Align[
                cute.struct.MemRange[self.dtype, cosize_sP], 1024
            ]
            sdS: cute.struct.Align[
                cute.struct.MemRange[self.dtype, cosize_sdS], 1024
            ]
    ...
    def __call__(
        ...
        mQ, mK, mV, mdK, mdV, mdO = [
            utils.select(t, layout_transpose)
    ...
    def load(
        ...
        if warp_idx_in_wg == 0:
            ...
            while work_tile.is_valid_tile:
                ...
                gQ = cute.local_tile(
                    mQ_cur, (self.tile_m, self.tile_hdim), (None, 0)
                )
    ...
    def mma(
        ...
        wg_mma_dQ = tiled_mma_dQ.get_slice(
            warp_group_thread_layout(warp_group_idx)
        )
        # S = Q @ K.T
        tSrQ, tSrK = mma_partition_fragment_AB(
            wg_mma_SdP, sQ, sK, self.SdP_swapAB
        )
        # dP = dO @ V.T
        ...
        tdQrdS, tdQrKt = mma_partition_fragment_AB(
            wg_mma_dQ, sdS, sKt, self.dQ_swapAB
        )
        ...
        tLSEsLSE = utils.make_acc_tensor_mn_view(
            thr_mma_SdP.partition_C(sLSE_mma)
        )[LSEslice]
        tLSEsdPsum = utils.make_acc_tensor_mn_view(
    ...
    def mma_one_m_block(
        self,
        m_block: Int32,
        consumer_state_Q: cutlass.pipeline.PipelineState
        ...
        tdVrP = utils.cvt_f16(
            utils.make_acc_tensor_frgA_view(acc_S), self.dtype
        )
        # R2S for P
        if const_expr(not self.mma_dkv_is_rs):
            ...
            cute.copy(
                smem_thr_copy_PdS, tPrP, tPsP[None, None, None, smem_idx_PdS]
            )

        # (4) [Pointwise 2] dS = P*(dP-dPsum)
        warpgroup.wait_group(0)
        acc_dP_mn = utils.make_acc_tensor_mn_view(
        ...
        tdKrdS = utils.cvt_f16(
            utils.make_acc_tensor_frgA_view(acc_dP), self.dtype
        )
        ...
        if const_expr(
            ...
            or (self.PdS_stage == 1 and self.mma_dkv_is_rs)
        ):
