...
if paddle.is_compiled_with_cuda():
    ...
    HYBRID_EP_HINT = (
        "For developers: guard imports with `is_hybrid_ep_available()` and only call `paddlefleet_ops.ops.hybrid_ep` when HybridEP branch enabled.\n"
...
FLASH_MASK_HINT = (
    "For developers: guard imports with `is_flash_mask_available()` and only call `paddlefleet_ops.ops.flash_mask` when flag branch enabled.\n"
...
def _safe_load_ecosystem_lib(
    ...
    with ModuleContext(lib_names, ops_dir):
        try:
            ...
            patch_module_namespace(lib_name, "paddlefleet_ops.ops.")
            if extra_libs_name:
...
if paddle.is_compiled_with_cuda():
    ...
    if is_hybrid_ep_available():
        ...
        logger.warning(warning)
        blocked_import_messages["paddlefleet_ops.ops.hybrid_ep"] = error
...
def __getattr__(name):
    module_name = f"paddlefleet_ops.ops.{name}"
