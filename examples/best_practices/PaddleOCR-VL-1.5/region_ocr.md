...
本教程旨在提供基于 PaddleFleet 微调 PaddleOCR-VL-1.5 模型适配区域识别任务的微调教程，使其能够更好地适应特定领域的文档结构与内容识别需求，资源需求和运行耗时见下方表格：
...
## 任务准备
### 模型准备
PaddleFleet 通过在训练配置文件中指定字段`model_name_or_path`来设置所用的模型。启动训练时如果本地没有该模型的缓存，那么 PaddleFleet 会自动下载模型并加载使用。
...
<div align="center">
  <img width="500" alt="region_train_example" src="https://github.com/forBlank/PaddleFleet/blob/paddleocr_vl_v15_doc/examples/best_practices/PaddleOCR-VL-1.5/assets/region_train_example.jpg" />
</div>
...
如果您想要基于自己的数据集进行训练，请参考 [PaddleFleet - 数据集格式文档](https://github.com/PaddlePaddle/PaddleFleet/blob/develop/docs/zh/dataset_format.md#24-%E5%A4%9A%E6%A8%A1%E6%80%81%E6%8C%87%E4%BB%A4%E5%BE%AE%E8%B0%83sft%E6%95%B0%E6%8D%AE%E6%A0%BC%E5%BC%8F) 准备数据。
...
除了自定义模板的格式，PaddleFleet 还支持自定义模板的多模态数据处理插件，包括：自定义数据增强、自定义多模态 Token 以及替换形式，更多模板和多模态数据处理插件的自定义和使用请参考 [PaddleFleet - 注册 template&mm_plugin](https://github.com/PaddlePaddle/PaddleFleet/blob/develop/docs/zh/template_zh.md)。
...
paddlefleet-cli train examples/best_practices/PaddleOCR-VL-1.5/paddleocr-vl-v15_full_16k_region_config.yaml \
                        model_name_or_path=PaddlePaddle/PaddleOCR-VL-1.5 \
                        train_dataset_path=./region_visual/region_visual_train.jsonl \
                        eval_dataset_path=./region_visual/region_visual_val.jsonl \
                        pre_alloc_memory=29
...
PaddleFleet 默认使用机器上的全部 GPU，可以通过环境变量 `CUDA_VISIBLE_DEVICES` 设置 PaddleFleet 能够使用的 GPU。
...
<div align="center">
  <img width="500" alt="regionocr_train_loss" src="https://github.com/forBlank/PaddleFleet/blob/paddleocr_vl_v15_doc/examples/best_practices/PaddleOCR-VL-1.5/assets/regionocr_train_loss.png" />
</div>
...
```bash
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 \
paddlefleet-cli train examples/best_practices/PaddleOCR-VL-1.5/paddleocr-vl-v15_lora_16k_region_config.yaml \
                        model_name_or_path=PaddlePaddle/PaddleOCR-VL-1.5 \
                        train_dataset_path=./region_visual/region_visual_train.jsonl \
                        eval_dataset_path=./region_visual/region_visual_val.jsonl \
                        pre_alloc_memory=26
...
```bash
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 \
paddlefleet-cli export examples/best_practices/PaddleOCR-VL-1.5/paddleocr-vl-v15_lora_export_region.yaml \
    model_name_or_path=PaddlePaddle/PaddleOCR-VL-1.5 \
    output_dir=./PaddleOCR-VL-1.5-SFT-RegionOCR-lora
```
...
<div align="center">
  <img width="500" alt="region_test_example" src="https://github.com/forBlank/PaddleFleet/blob/paddleocr_vl_v15_doc/examples/best_practices/PaddleOCR-VL-1.5/assets/region_test_example.jpg" />
</div>
...
from paddlefleet.transformers import AutoModelForConditionalGeneration, AutoProcessor
...
from paddlefleet.transformers import AutoModelForConditionalGeneration, AutoProcessor, AutoConfig
from paddlefleet.generation import GenerationConfig

def parse_args():
    parser = argparse.ArgumentParser(description="PaddleFleet & PaddleOCR-VL Model Evaluation Script")
...
上述直接运行的评估脚本基于 PaddleFleet 原生推理，测试集较大的情况下，推理速度可能较慢。推荐使用 FastDeploy 等高性能推理引擎将微调后的模型部署为 API 服务，通过高并发请求进行批量推理评估，详见[部署推理](#部署推理)。
...
### 部署推理
部署 PaddleOCR-VL-1.5 模型，请参考 [PaddleFleet - 模型部署文档](https://github.com/PaddlePaddle/PaddleFleet/blob/develop/docs/zh/deployment_guide.md) 和 [FastDeploy - PaddleOCR-VL-0.9B Best Practices](https://paddlepaddle.github.io/FastDeploy/zh/best_practices/PaddleOCR-VL-0.9B/)。
...
### 更多硬件上的使用说明
PaddleOCR-VL-1.5 支持基于昆仑芯 P800 和天数智芯 150s 进行微调。本教程选用了规模较大的数据集，在部分硬件上运行时间可能较长，如果希望**快速跑通流程**，或仅需验证**国产硬件环境的兼容性**，建议优先参考我们的 [PaddleFleet - 基于 PaddleOCR-VL 微调实现孟加拉语识别能力](https://github.com/PaddlePaddle/PaddleFleet/tree/develop/examples/best_practices/PaddleOCR-VL)，该教程使用精简数据集，可在短时间内完成微调全链路，助力迅速掌握多硬件适配技巧。
