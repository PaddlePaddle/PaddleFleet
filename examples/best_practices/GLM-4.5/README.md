...
PaddleFleet 提供冻结主干的 MTP 权重训练，可以基于无 MTP（或只有一层 MTP 权重）的模型进行进 N 步 MTP 能力训练，让模型具有多步 MTP 能力。
...
train_dataset_path: ./tests/formers/fixtures/dummy/sft/train.jsonl
train_dataset_prob: "1.0"
eval_dataset_path: ./tests/formers/fixtures/dummy/sft/eval.jsonl
...
```shell
NNODES={num_nodes} MASTER_ADDR={your_master_addr} MASTER_PORT={your_master_port} RANK={rank} CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 paddlefleet-cli train mtp_depth_3.yaml
```
