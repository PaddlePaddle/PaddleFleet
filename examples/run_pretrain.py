...
from paddlefleet.transformers import AutoConfig, AutoTokenizer
from paddlefleet.transformers.configuration_utils import (
    LlmMetaConfig,
    llmmetaclass,
)
from paddlefleet.utils.batch_sampler import DistributedBatchSampler
...
class ModelArguments:
    ...
    model_name_or_path: str = field(
        ...
        metadata={
            "help": "Path to pretrained model or model identifier from https://paddlefleet.readthedocs.io/zh/latest/model_zoo/transformers.html"
        },
    )
    ...
    continue_training: bool = field(
        default=False,
        metadata={
            "help": "Pre-training from existing paddlefleet model weights. Default False and model will train from scratch. If set True, the model_name_or_path argument must exist in the paddlefleet models."
        },
    )
    ...
    def _collate_data(batch, stack_fn=Stack()):
        ...
        for batch_sequence in batch:
            ...
            padded_position_ids = np.array(
                [list(chain.from_iterable(batch_sequence["position_ids"]))[:-1]]
            )
            return_list.append(
                [
                    padded_token_ids,
