...
def load_longproc_data_for_helmet(
    dataset: str,
    path="longproc_addon/longproc/data",
    ...
    if max_test_samples is not None:
        packed_data = packed_data.shuffle(seed=seed).select(
            range(min(max_test_samples, len(packed_data)))
        )
...
def _test_load_all():
    def test_loading(dataset):
        data, eval_func = load_longproc_data(
            dataset, "longproc_addon/longproc/data"
        )
        ...
        print(
            f"Max output chars: {max([len(d['reference_output']) for d in data])}"
        )
    ...
    [
        test_loading(d)
        for d in ["html_to_tsv_0.5k", "html_to_tsv_2k", "html_to_tsv_8k"]
    ]
    ...
    [
        test_loading(d)
        for d in ["tom_tracking_0.5k", "tom_tracking_2k", "tom_tracking_8k"]
    ]

    [
        test_loading(d)
        for d in ["countdown_0.5k", "countdown_2k", "countdown_8k"]
    ]
