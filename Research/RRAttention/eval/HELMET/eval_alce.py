...
def _torch_cuda_device_id():
    if not torch.cuda.is_available():
        return -1
    return int(os.environ.get("LOCAL_RANK", 0))


def _empty_torch_cache():
    if torch.cuda.is_available():
        torch.cuda.empty_cache()


def all_gather_object(results):
    rank = dist.get_rank()
...
def compute_rouge(data, allgather=False):
    ...
    if allgather:
        ...
        r1 = all_gather_object(r1)
        r2 = all_gather_object(r2)
...
def compute_qa(data, allgather=False, qa_model_name_or_path=None):
    ...
    if allgather:
        em = all_gather_object(em)
        f1 = all_gather_object(f1)
        bins = all_gather_object(bins)
...
def compute_mauve(data):
    ...
    out = mauve.compute_mauve(
        p_text=human_data,
        q_text=model_data,
        device_id=_torch_cuda_device_id(),
        max_text_length=512,
        verbose=True,
        batch_size=8,
...
def compute_claims(data, allgather=False, autoais_model_name_or_path=None):
    global autoais_model, autoais_tokenizer, AUTOAIS_MODEL
    ...
    if allgather:
        scores = all_gather_object(scores)
...
def compute_autoais(
    ...
    sent_mcite_overcite = []
    autoais_log = []
    citation_position_count = defaultdict(int)
    for item in tqdm(data):
    ...
    if allgather:
        ...
        sent_mcite = all_gather_object(sent_mcite)
        sent_mcite_support = all_gather_object(sent_mcite_support)
        sent_mcite_overcite = all_gather_object(sent_mcite_overcite)

        ais_scores = all_gather_object(ais_scores)
        ais_scores_prec = all_gather_object(ais_scores_prec)

        rank = dist.get_rank()
        world_size = dist.get_world_size()
        # 数据并行结果汇总
        gathered_results = [None for _ in range(world_size)]
        dist.all_gather_object(gathered_results, citation_position_count)
        citation_position_count = defaultdict(lambda: 0)
        for res in gathered_results:
            for k in res:
                citation_position_count[k] += res[k]

    sent_total = sum(sent_total)
    sent_mcite = sum(sent_mcite)
    sent_mcite_support = sum(sent_mcite_support)
...
def compute_qampari_f1(data, cot=False, allgather=False):
    prec = []
    rec = []
    rec_top5 = []
    f1 = []
    ...
    if allgather:
        ...
        prec = all_gather_object(prec)
        rec = all_gather_object(rec)
        rec_top5 = all_gather_object(rec)
        f1 = all_gather_object(f1)
        f1_top5 = all_gather_object(f1_top5)
...
def main(args=None):
    ...
    if args.qa_model_name_or_path is not None:
        global QA_MODEL
        QA_MODEL = args.qa_model_name_or_path

    if args.autoais_model_name_or_path is not None:
        global AUTOAIS_MODEL
        AUTOAIS_MODEL = args.autoais_model_name_or_path
