...
speed_metrics = ['sparse_ratios', 'e2e_ms', 'ttft_ms', 'attn_ms', 'estimate_func_ms']
...
class arguments:
    ...
    def get_averaged_metric(self):
        ...
        s = {m : v * (100 if m == "gpt-4-f1" else 1) * (100/3 if m == "gpt-4-score" else 1) if (m != "sparse_ratios" and '_ms' not in m) else v for m, v in s.items()}
...
if __name__ == "__main__":
    ...
    models_configs = [
            ...
            {"model": "Meta-Llama-3.1-8B-Instruct/rrattn_0.9", "training_length": 131072},
            {"model": "Meta-Llama-3.1-8B-Instruct/rrattn_0.95", "training_length": 131072},
            {"model": "Meta-Llama-3.1-8B-Instruct/rrattn_0.99", "training_length": 131072},
            {"model": "Meta-Llama-3.1-8B-Instruct/flex_0.9", "training_length": 131072},
            {"model": "Meta-Llama-3.1-8B-Instruct/flex_0.95", "training_length": 131072},
            {"model": "Meta-Llama-3.1-8B-Instruct/flex_0.99", "training_length": 131072},
            {"model": "Meta-Llama-3.1-8B-Instruct/xattn_0.9", "training_length": 131072},
            {"model": "Meta-Llama-3.1-8B-Instruct/xattn_0.95", "training_length": 131072},
            {"model": "Meta-Llama-3.1-8B-Instruct/xattn_0.99", "training_length": 131072},
            ...
            {"model": "ERNIE-4.5-21B-A3B-PT/rrattn_0.95", "training_length": 131072},
            {"model": "ERNIE-4.5-21B-A3B-PT/rrattn_0.99", "training_length": 131072},
    ]
    ...
    lf_df = all_df.pivot_table(index=["input_max_length", "model", "tag", ], columns="dataset_simple", values="metric", sort=False)
    lf_df = lf_df.reset_index()

    for m in speed_metrics:
        avg_tmp_df = all_df.dropna(subset=[m]).groupby(
            ["input_max_length", "model", "tag"]
        )[m].mean().reset_index()
        lf_df = pd.merge(lf_df, avg_tmp_df, on=["input_max_length", "model", "tag"], how="left")
    ...
    cols_to_show = ['input_max_length', 'model', 'tag'] + speed_metrics + ['Avg.', 'Recall', 'RAG', 'ICL', 'Cite', 'Re-rank', 'LongQA'] 
    cols_to_float = speed_metrics + ['Avg.', 'Recall', 'RAG', 'ICL', 'Cite', 'Re-rank', 'LongQA']
    ...
    lf_df['model_order'] = pd.Categorical(lf_df['model'], categories=model_order, ordered=True)
    lf_df['length_order'] = pd.Categorical(lf_df['input_max_length'], categories=length_order, ordered=True)
    lf_df = lf_df.sort_values(['length_order', 'model_order']).drop(['model_order', 'length_order'], axis=1)

    lf_df[cols_to_float] = lf_df[cols_to_float].map(lambda x: f"{x:.2f}" if isinstance(x, (int, float)) else x)
    ...
    print(tabulate(rows, headers=cols_to_show, tablefmt='pipe', showindex=False, colalign=colalign))
