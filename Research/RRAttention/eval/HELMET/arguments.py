...
def parse_arguments():
    ...
    parser.add_argument("--record_attn_ms", action="store_true", help="record attn time(ms)")

    # model setting
    parser.add_argument("--model_name_or_path", type=str, default=None)
    parser.add_argument("--model_type", type=str, default="auto", choices=["auto", "llama", "qwen", "qwen3", "ernie"], help="PaddleFormers model family used by the Paddle worker")
    ...
    parser.add_argument("--paddle_worker_clear_cache_per_request", action="store_true", help="clear Paddle CUDA cache after every generation request")
    parser.add_argument("--paddle_worker_timeout_s", type=float, default=1800.0, help="max seconds to wait for a Paddle worker response before terminating it")
    ...
    parser.add_argument("--data_root_dir", type=str, default=None, help="data root dir")
    parser.add_argument("--data_cache_dir", type=str, default="./cache_data", help="data root dir")
    ...
    # alce eval settings
    parser.add_argument("--alce_no_rouge", action="store_true", help="Do not evaluate ROUGE score")
    parser.add_argument("--alce_qa", action="store_true", help="Use the QA model")
    parser.add_argument("--alce_mauve", action="store_true", help="Use the mauve score model")
    parser.add_argument("--alce_citations", action="store_true", help="Evaluation with citation")
    parser.add_argument("--alce_at_most_citations", type=int, default=3, help="At most take this many documents (mostly for precision)")
    parser.add_argument("--alce_claims_nli", action="store_true", help="Use claims for ELI5")
    # QAMPARI
    parser.add_argument("--alce_cot", action="store_true", help="For QAMPARI, try to find colon and separate the COT and answer listing")

    # misc
    parser.add_argument("--debug", action="store_true", help="for debugging")
    parser.add_argument("--save_outputs", action="store_true", help="save per-sample outputs in the JSON result for debugging")
