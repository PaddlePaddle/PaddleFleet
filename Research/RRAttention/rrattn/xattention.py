...
from .kernels_xattn import flat_group_gemm_fuse_reshape, softmax_fuse_block_sum
...
def xattn_estimate(
    ...
    **kwargs,
    ...
    if use_triton and not can_use_triton_kernels():
        raise RuntimeError("xattn v1 Triton kernels require a CUDA gpu device")
    ...
    if keep_sink:
        simple_masks[:, :, :, 0] = True
    if keep_recent:
        eye_matrix = paddle.eye(q_block_num, device=simple_masks.device, dtype=paddle.bool)
