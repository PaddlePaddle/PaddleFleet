...
def find_blocks_chunked(
    ...
    if mode == "decode" and not decoding:
        ...
        if causal:
            mask[:, :, :, current_index : current_index + chunk_num] = paddle.tril(
                paddle.ones(1, head_num, chunk_num, chunk_num, device=input_tensor.device)
            )
    ...
    if causal:
        ...
        mask[:, :, :, current_index : current_index + chunk_num] = (
            paddle.eye(chunk_num, device=mask.device).astype(paddle.bool).unsqueeze(0).unsqueeze(0).expand(
                1, head_num, chunk_num, chunk_num
            )
        )
        ...
        sorted_values = paddle.concat(
            [
                paddle.zeros((batch_size, head_num, chunk_num, 1), device=input_tensor.device),
                paddle.where(mask, input_tensor, paddle.zeros_like(input_tensor)).sum(dim=-1, keepdim=True),
                sorted_values[:, :, :, :-2],
            ],
            dim=-1,
        )
        ...
        mask[:, paddle.arange(mask.shape[1], device=mask.device).unsqueeze(dim=-1), index] = True
        mask = mask.view(batch_size, head_num, chunk_num, block_num)
    else:
        mask = paddle.zeros_like(input_tensor, dtype=paddle.bool)
        sorted_values, index = paddle.compat.sort(input_tensor, dim=-1, descending=True)
        sorted_values = sorted_values.to(input_tensor.device)
        cumulative_sum_without_self = paddle.concat(
            [
                paddle.zeros((batch_size, head_num, chunk_num, 1), device=input_tensor.device),
    ...
    if causal:
        if decoding:
            ...
            lambda_mask = paddle.zeros_like(input_tensor, dtype=paddle.bool, device=input_tensor.device)
            lambda_mask[:, :, :, 0] = 1
            lambda_mask[:, :, :, current_index : current_index + chunk_num] = (
                paddle.eye(chunk_num, device=lambda_mask.device).astype(paddle.bool).unsqueeze(0).unsqueeze(0).expand(
                    1, head_num, chunk_num, chunk_num
                )
            )
